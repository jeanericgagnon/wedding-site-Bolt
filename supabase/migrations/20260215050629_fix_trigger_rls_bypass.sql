/*
  ...
  ## Overview
  The trigger function needs to properly bypass RLS policies when creating demo data.
  The issue is that RLS policies check auth.uid(), which may not be properly set during
  the trigger execution context.

  ## Changes
  - Recreate the trigger function with proper RLS bypass using SET local configuration
  - Ensure the function can insert data into all tables regardless of RLS policies

  ## Security
  - Uses SECURITY DEFINER to run with elevated privileges
  - Only creates demo data for the specific demo email address
*/
