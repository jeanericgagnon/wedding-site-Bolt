/*
  # Fix Demo Trigger RLS Bypass

  1. Issue
    - Trigger function runs during user signup before auth context is fully set
    - RLS policies on tables block the inserts from the trigger
    - Need to grant explicit permissions to the trigger function

  2. Solution
    - Grant necessary permissions to the function
    - Use SECURITY DEFINER to bypass RLS
    - Explicitly set search_path for security
*/
...
BEGIN
  ...
  IF NEW.email = 'demo@dayof.love' THEN
    ...
    -- Insert with RLS bypassed via SECURITY DEFINER
    ...
    -- Create guests (68 confirmed)
    INSERT INTO public.guests (wedding_site_id, name, first_name, last_name, email, rsvp_status, meal_preference, invite_token, invited_to_ceremony, invited_to_reception)
    ...
    INSERT INTO public.guests (wedding_site_id, name, first_name, last_name, email, rsvp_status, invite_token, invited_to_ceremony, invited_to_reception)
    ...
    -- Create events
    INSERT INTO public.itinerary_events (wedding_site_id, event_name, description, event_date, start_time, location_name, display_order)
    VALUES 
    ...
    SELECT id INTO v_welcome_dinner_id FROM public.itinerary_events WHERE wedding_site_id = v_demo_site_id AND event_name = 'Welcome Dinner';
    SELECT id INTO v_ceremony_id FROM public.itinerary_events WHERE wedding_site_id = v_demo_site_id AND event_name = 'Ceremony';
    SELECT id INTO v_reception_id FROM public.itinerary_events WHERE wedding_site_id = v_demo_site_id AND event_name = 'Reception';
    SELECT id INTO v_brunch_id FROM public.itinerary_events WHERE wedding_site_id = v_demo_site_id AND event_name = 'Sunday Brunch';

    -- Event invitations for ceremony and reception
    INSERT INTO public.event_invitations (event_id, guest_id)
    SELECT v_ceremony_id, id FROM public.guests WHERE wedding_site_id = v_demo_site_id;
    
    INSERT INTO public.event_invitations (event_id, guest_id)
    SELECT v_reception_id, id FROM public.guests WHERE wedding_site_id = v_demo_site_id;

    -- RSVPs for confirmed guests
    INSERT INTO public.event_rsvps (event_invitation_id, attending)
    SELECT ei.id, true
    FROM public.event_invitations ei
    JOIN public.guests g ON g.id = ei.guest_id
    ...
    -- Main RSVPs
    INSERT INTO public.rsvps (guest_id, attending, meal_choice)
    SELECT id, true, meal_preference
    FROM public.guests
    WHERE wedding_site_id = v_demo_site_id
    AND rsvp_status = 'confirmed';

    INSERT INTO public.rsvps (guest_id, attending)
    SELECT id, false
    FROM public.guests
    WHERE wedding_site_id = v_demo_site_id
    ...
    -- Photos
    INSERT INTO public.photos (wedding_site_id, url, caption, category, display_order)
    VALUES 
    ...
    -- Registry items
    INSERT INTO public.registry_items (wedding_site_id, item_name, price, store_name, quantity_needed, quantity_purchased, priority)
    VALUES 
    ...
    -- Site content
    INSERT INTO public.site_content (wedding_site_id, section_type, title, content, display_order)
    VALUES 
      (v_demo_site_id, 'story', 'Our Story', 'We met five years ago and fell in love!', 1);

    -- Messages
    INSERT INTO public.messages (wedding_site_id, subject, body, channel, audience_filter, recipient_count)
    VALUES 
