/*
  # Fix Demo Trigger Household ID Type

  ## Overview
  Fixes the create_demo_wedding_data trigger to properly convert household_id UUIDs to text type.

  ## Changes
  - Updates the trigger function to cast household IDs to text before inserting into guests table
  - The guests.household_id column is of type text, but we were inserting UUID values directly

  ## Security
  - Maintains SECURITY DEFINER to allow trigger to bypass RLS when creating demo data
*/
...
BEGIN
  ...
  IF NEW.email = 'demo@dayof.love' THEN
    ...
    -- Generate household IDs as text
    household_1 := gen_random_uuid()::text;
    household_2 := gen_random_uuid()::text;
    household_3 := gen_random_uuid()::text;
