/*
  # Fix Demo with Trigger

  ## Overview
  Removes the manual auth.users creation and instead uses a trigger that creates
  demo data when a user signs up with demo@dayof.love.

  ## Changes
  1. Removes manual demo user from auth.users
  2. Clears any existing demo data
  3. Creates trigger function that generates consistent demo data
*/

-- Remove manually created demo user
DELETE FROM auth.users WHERE email = 'demo@dayof.love';

-- Create trigger function for demo data
