...
const SUBSCRIPTION_SIGNIN_REQUIRED_COPY = "Please sign in to continue checkout.";
const SUBSCRIPTION_DETAILS_REQUIRED_COPY = "Choose a site and return links before continuing checkout.";
const SUBSCRIPTION_SITE_UNAVAILABLE_COPY = "This site is not available for checkout.";
const SUBSCRIPTION_ALREADY_RECURRING_COPY = "This site is already on recurring billing.";
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (!authHeader) {
      return json({ error: SUBSCRIPTION_SIGNIN_REQUIRED_COPY }, 401);
    }
    ...
    if (!wedding_site_id || !success_url || !cancel_url) {
      return json({ error: SUBSCRIPTION_DETAILS_REQUIRED_COPY }, 400);
    }
    ...
    if (siteError || !site) {
      return json({ error: SUBSCRIPTION_SITE_UNAVAILABLE_COPY }, 404);
    }

    if (site.billing_type === "recurring") {
      return json({ error: SUBSCRIPTION_ALREADY_RECURRING_COPY }, 409);
    }
