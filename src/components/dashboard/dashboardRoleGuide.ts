...
export function buildDashboardRoleGuide(role: DashboardRoleGuideRole): DashboardRoleGuideModel {
  ...
  if (role === 'coordinator') {
    return {
      ...
      nextMove: 'Start in Overview, then open Coordinator Mode or Itinerary once the real day-of pressure is visible.',
      decisionRule: 'If it affects guest-facing flow or day-of calm, handle it now. If it can wait until after the pressure passes, let it wait.',
      watchout: 'Do not absorb broader planning truth while the room is under pressure. If a choice will outlive the day-of window, capture it and return it after the floor is calm.',
      sequence: [
        {
          id: 'orient',
          status: 'current',
          title: 'Spot the day-of pressure',
          detail: 'Use Overview first so you can tell whether the room needs timeline clarity, guest movement help, or active guest messaging.',
        },
        {
          id: 'enter',
          status: 'next',
        ...
        },
        {
          ...
          detail: 'After the active lane is steady, only reopen lower-pressure cleanup if the room truly has space for it.',
        },
      ],
    };
  }
