...
export const profileToOnboardingForm = (profile: WeddingProfile) => ({
  ...
  guestExperience: '', // legacy compatibility only
  ...
  registryLink: '', // legacy compatibility only
...
export const onboardingFormToProfile = (formData: OnboardingFormShape): WeddingProfile => {
  ...
  const profile: WeddingProfile = {
    ...
    registry: {
      url: formData.registryIntent || '',
      status: (formData.registryIntent || '').trim() ? 'linked' : 'missing',
    },
    design: {
