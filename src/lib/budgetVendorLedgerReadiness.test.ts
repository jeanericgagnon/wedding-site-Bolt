...
describe('budget vendor ledger readiness', () => {
  ...
  it('exports a combined budget and vendor ledger csv', () => {
    ...
    expect(csv).toContain('"Internal Rating","Rating Status","Private Rating Notes","Contact Ready","Due Date Ready","File Count","Milestone Count","Linked Budget Count","Linked Budget Lines","Linked Budget Categories","Linked Budget Due Dates","Linked Budget Notes","Linked Budget Statuses","Linked Budget Estimated","Linked Budget Actual","Linked Budget Paid","Contract Gap","Paid Gap","Ledger Issue Count","Ledger Issues","Notes"');
    expect(csv).toContain('"Contact Ready","Due Date Ready","File Count","Milestone Count","Linked Budget Count","Linked Budget Lines","Linked Budget Categories","Linked Budget Due Dates","Linked Budget Notes","Linked Budget Statuses","Linked Budget Estimated","Linked Budget Actual","Linked Budget Paid","Contract Gap","Paid Gap","Ledger Issue Count","Ledger Issues"');
    ...
    expect(csv).toContain('"5","Booked","Strong fit","Yes","Yes","1","1","1","Photo deposit","Photography","2026-05-20","Booked","Partially paid","5000","5000","2500","0","0","0","","Booked"');
  });
  ...
  it('exports reconciliation blockers alongside vendor and budget handoff rows', () => {
    ...
    expect(csv).toContain('"No","No","0","0","1","Floral balance","Florals","2026-05-20","Short by final contract amount","Partially paid","2500","2500","500","500","500","6"');
  ...
  it('reconciles vendor totals, files, and milestones against linked budget rows', () => {
    ...
    expect(reconciliation.rows[0]?.issues).toContain('Open balance has no saved due date');
