...
function normalizeDraftDateValue(value: string) {
  ...
  const isoTimestampMatch = normalizedOrdinalValue.match(/^(\d{4})-(\d{2})-(\d{2})(?:[T\s].*|[+-]\d{2}:?\d{2}|Z|\s+[A-Za-z]{2,5})?$/);
  ...
  const timestampSlashMatch = normalizedOrdinalValue.match(/^(\d{4})\/(\d{1,2})\/(\d{1,2})(?:[T\s].*|[+-]\d{2}:?\d{2}|Z|\s+[A-Za-z]{2,5})?$/);
