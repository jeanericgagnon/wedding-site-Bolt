...
describe('name change insurance packet snapshot', () => {
  it('builds a structured insurance update packet payload', () => {
    ...
    expect(snapshot.fields.find((field) => field.fieldKey === 'policyHolder.currentMiddleName')).toMatchObject({ value: 'Marie' });
    expect(snapshot.fields.find((field) => field.fieldKey === 'policyHolder.newFirstName')).toMatchObject({ value: 'Alex' });
    expect(snapshot.fields.find((field) => field.fieldKey === 'policyHolder.newMiddleName')).toMatchObject({ value: 'Marie' });
