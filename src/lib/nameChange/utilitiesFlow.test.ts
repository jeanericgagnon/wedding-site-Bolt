...
import { buildNameChangeUtilitiesExecutionSnapshot } from './utilitiesFlow';
...
describe('name change utilities execution snapshot', () => {
  it('marks utilities/lease updates ready when primary ID is moving and support docs exist', () => {
    ...
    const snapshot = buildNameChangeUtilitiesExecutionSnapshot(makeCase(), documents, [], plan);
  ...
  it('blocks utilities/lease updates when primary photo ID has not started moving', () => {
    const snapshot = buildNameChangeUtilitiesExecutionSnapshot(makeCase(), [], []);
    expect(snapshot.ready).toBe(false);
    expect(snapshot.blockers).toContain('Utilities/lease record updates usually go smoother once your primary photo ID is moving or already updated.');
