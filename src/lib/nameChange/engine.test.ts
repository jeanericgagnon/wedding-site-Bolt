...
describe('name change engine', () => {
  ...
  it('builds federal-first workflow with passport and employer follow-through', () => {
    ...
    expect(plan.summary.accountUpdateTemplates?.find((template) => template.id === 'template-travel')?.body).toContain('I am not sending the final packet yet and mainly need the intake path for the next proof hop.');
    ...
    expect(plan.summary.accountUpdateTemplates?.find((template) => template.id === 'template-digital-identity')?.body).toContain('I am not sending the final packet yet and mainly need the intake path for the next proof hop.');
  ...
  it('surfaces blockers when the legal proof doc is missing', () => {
    ...
    expect(plan.summary.accountUpdateTemplates?.find((template) => template.id === 'template-tax')?.body).toContain('Current blocker: SSA pending.');
    expect(plan.summary.accountUpdateTemplates?.find((template) => template.id === 'template-tax')?.body).toContain('Do not send yet; the legal proof chain still needs to clear before SSA-backed tax updates can stick.');
