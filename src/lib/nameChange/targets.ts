...
export const NAME_CHANGE_EXECUTION_TARGETS: Record<'courtOrder' | 'ssa' | 'dmv' | 'passport' | 'employer' | 'taxes' | 'legalGovernment' | 'banks' | 'insurance' | 'medical' | 'utilities' | 'courtesy' | 'voter' | 'tsa' | 'licenses', NameChangeExecutionTargetDefinition> = {
  courtOrder: {
    key: 'courtOrder',
  ...
  passport: {
    ...
    checklistSpecs: [
      ...
      },
      {
        ...
        attentionReason: 'Passport follow-through still needs the county, certificate number, and issuing authority from the out-of-state marriage certificate.',
      ...
      },
      {
        ...
        label: 'Citizenship proof intake ready for first-passport work',
        kind: 'requirement',
        requirementKey: 'citizenship-proof-intake',
        nextActionCategory: 'document',
        blocksReady: true,
        missingReason: 'Citizenship proof intake has not been evaluated for passport prep yet.',
        satisfiedReason: 'Citizenship proof intake is ready for the modeled passport path.',
      },
    ],
  },
  employer: {
  ...
  legalGovernment: {
    ...
    prerequisiteRules: [
      {
        key: 'legal-proof-ready-for-government-follow-through',
        label: 'Legal proof available for government follow-through',
        required: true,
        requiredStepId: 'federal-ssa',
        requiredStatuses: ['in_progress', 'complete'],
        missingReason: 'County recorder and immigration updates should wait until legal proof is grounded through SSA-level identity confirmation.',
        attentionReason: 'SSA is moving, which is enough to prepare county recorder and immigration follow-through.',
        satisfiedReason: 'SSA is already grounded, so county recorder and immigration follow-through can proceed cleanly.',
      },
      {
        key: 'photo-id-identity-progress',
        label: 'Photo ID progress for government record sync',
        required: false,
        requiredStepId: 'state-dmv',
        requiredStatuses: ['in_progress', 'complete'],
        missingReason: 'Updated photo ID is not moving yet, so some county recorder or immigration updates may need extra manual proof handling.',
        attentionReason: 'Photo ID is moving, which helps government record sync land with fewer manual follow-ups.',
        satisfiedReason: 'Photo ID is already moving for government record sync.',
      },
    ],
    autofillTargetFields: [
      'applicant.current_first_name',
    ...
    checklistSpecs: [
      {
        key: 'legal-proof-document',
        label: 'Legal proof ready for county recorder and immigration follow-through',
        kind: 'requirement',
        requirementKey: 'legal-proof-document',
        missingReason: 'Legal proof requirement not evaluated.',
        satisfiedReason: 'Legal proof document is ready for county recorder and immigration follow-through.',
      },
      {
        key: 'identity-document-coverage',
        label: 'Identity document coverage',
        ...
        requirementKey: 'identity-document-coverage',
        missingReason: 'Identity coverage requirement not evaluated.',
        satisfiedReason: 'Identity document coverage is ready for government record alignment.',
      },
      {
        key: 'county-context',
        label: 'County / local jurisdiction context',
        kind: 'requirement',
        requirementKey: 'county-context',
        missingReason: 'County and local filing context has not been evaluated yet.',
        satisfiedReason: 'County and local filing context is available for government record alignment.',
      },
      {
        key: 'canonical-extraction-alignment',
        label: 'Canonical vs extracted values aligned',
        ...
        requirementKey: 'canonical-extraction-alignment',
        missingReason: 'Canonical/extracted alignment has not been evaluated yet.',
        attentionReason: 'Structured case truth and extracted document values still disagree for government record alignment.',
        satisfiedReason: 'Structured case truth and extracted document values are aligned for government record alignment.',
      },
      {
        key: 'current-legal-name',
        label: 'Current legal name available for government record alignment',
        kind: 'field_presence',
        targetFields: ['applicant.current_first_name', 'applicant.current_last_name'],
        conditionalTargetFields: ['applicant.current_middle_name'],
        missingReason: 'Current legal name fields are still incomplete for government record alignment.',
        satisfiedReason: 'Current legal name is available for government record alignment.',
      },
      {
        key: 'target-legal-name',
        label: 'Target legal name available for government record alignment',
        kind: 'field_presence',
        targetFields: ['applicant.target_first_name', 'applicant.target_last_name'],
        conditionalTargetFields: ['applicant.target_middle_name'],
        missingReason: 'Target legal name fields are still missing for government record alignment.',
        satisfiedReason: 'Target legal name is available for government record alignment.',
      },
      {
        key: 'government-support-doc',
        label: 'Government-record supporting identity document intake',
        kind: 'document_support',
        documentKinds: ['social_security_card', 'current_drivers_license', 'current_passport', 'court_order', 'marriage_certificate'],
        missingReason: 'No government-record supporting identity document is represented in intake yet.',
        satisfiedReason: 'A government-record supporting document exists in intake.',
      },
    ],
  },
  ...
  medical: buildPhotoIdInstitutionTarget({
    ...
    primaryRule: {
      ...
      requiredStepId: 'state-dmv',
      requiredStatuses: ['in_progress', 'complete'],
      missingReason: 'Medical office record updates usually go smoother once your primary photo ID is moving or already updated.',
      attentionReason: 'Primary photo ID is moving, which supports medical-office follow-through.',
      satisfiedReason: 'Primary photo ID is already moving or complete for medical-office follow-through.',
    },
    secondaryRule: {
      ...
      requiredStatuses: ['in_progress', 'complete'],
      missingReason: 'Insurance rollout is not moving yet, so care records may be the first healthcare lane to start.',
      attentionReason: 'Insurance rollout is already moving, which helps member records and care rosters stay aligned.',
  ...
  utilities: buildPhotoIdInstitutionTarget({
    ...
    secondaryRule: {
      ...
      requiredStepId: 'institution-banks',
      requiredStatuses: ['in_progress', 'complete'],
      missingReason: 'Bank updates have not started yet, so utilities or lease updates may be the first household account step.',
    ...
    prerequisiteRules: [
      {
        ...
        label: 'At least one related account step is moving',
        required: false,
        requiredStepId: 'institution-banks',
        requiredStatuses: ['in_progress', 'complete'],
        missingReason: 'No related account step is moving yet, so courtesy and social updates may be early.',
        attentionReason: 'A related account step is already moving, which makes courtesy and social updates easier near the end.',
        satisfiedReason: 'A related account step is already moving for tail-end courtesy and social updates.',
      },
    ],
    autofillTargetFields: [
      'applicant.current_first_name',
  ...
  tsa: {
    ...
    checklistSpecs: [
      ...
      },
      {
        ...
        requirementKey: 'out-of-state-marriage-certificate-grounding',
        missingReason: 'Out-of-state marriage certificate grounding has not been evaluated yet.',
        attentionReason: 'TSA / travel-profile follow-through still needs the county, certificate number, and issuing authority from the out-of-state marriage certificate.',
