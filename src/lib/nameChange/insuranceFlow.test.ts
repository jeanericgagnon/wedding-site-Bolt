...
import { buildNameChangeInsuranceExecutionSnapshot } from './insuranceFlow';
...
describe('name change insurance execution snapshot', () => {
  it('marks insurance packet ready when primary photo ID is moving and support docs exist', () => {
    ...
    const snapshot = buildNameChangeInsuranceExecutionSnapshot(makeCase(), documents, [], plan);
  ...
  it('blocks insurance packet when primary photo ID has not started moving', () => {
    const snapshot = buildNameChangeInsuranceExecutionSnapshot(makeCase(), [], []);
    expect(snapshot.ready).toBe(false);
    expect(snapshot.blockers).toContain('Insurance updates usually go smoother once your primary photo ID is moving or already updated.');
