...
import { buildNameChangeBankExecutionSnapshot } from './bankFlow';
...
describe('name change bank execution snapshot', () => {
  it('marks bank packet ready when primary photo ID is moving and support docs exist', () => {
    ...
    const snapshot = buildNameChangeBankExecutionSnapshot(makeCase(), documents, [], plan);
  ...
  it('blocks bank packet when primary photo ID has not started moving', () => {
    const snapshot = buildNameChangeBankExecutionSnapshot(makeCase(), [], []);
    expect(snapshot.ready).toBe(false);
    expect(snapshot.blockers).toContain('Banks are usually easiest once your primary photo ID is moving or already updated.');
