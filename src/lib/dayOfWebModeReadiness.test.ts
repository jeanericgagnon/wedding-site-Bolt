...
describe('day-of web mode readiness', () => {
  ...
  it('marks status board ready only when live handoff pieces are connected', () => {
    ...
    expect(board.summary).toBe('Guest hub status is connected for live day-of use. Core link coverage: 100% ready (4 of 4).');
    expect(board.items.find((item) => item.id === 'link-access')?.detail).toBe(
      'Guests can tell whether this hub link is public, invite-only, or guest-specific, plus which actions are unlocked from it: RSVP, Schedule, Directions and travel, and Photo upload. Core day-of coverage from this link is 100% ready: RSVP, Schedule, Directions and travel, and Photo upload.'
    );
  });
  ...
  it('calls out the main core-day-of gap when link visibility is connected but coverage is partial', () => {
    ...
    expect(board.summary).toBe('1 day-of status item is usable now; 5 stay planned or need setup. Core link coverage: 50% ready (2 of 4; 2 still missing). First blocker: Announcements.');
    expect(board.items.find((item) => item.id === 'link-access')?.detail).toBe(
