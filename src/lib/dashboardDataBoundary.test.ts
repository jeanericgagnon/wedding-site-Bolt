...
describe('dashboard data boundary guards', () => {
  ...
  it('loads itinerary events and event guest picker rows with explicit projections', () => {
    ...
    expect(uiStateHook).toContain('export function useItineraryDashboardUiState()');
  ...
  it('keeps overview intelligence persistence behind its service', () => {
    ...
    expect(routeSupport).toContain("const [dismissedIntelligenceIds, setDismissedIntelligenceIds] = useState<string[]>(() => {");
    expect(routeSupport).toContain("new URLSearchParams(window.location.search).get('proof') === '1'");
