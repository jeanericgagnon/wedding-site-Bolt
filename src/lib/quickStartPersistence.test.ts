...
describe('quickStartPersistence', () => {
  ...
  it('recovers closed restore progress from legacy string indexes when setup answers survive', () => {
    const normalized = normalizeQuickStartDraftSnapshot({
      currentIndex: '5',
      initialSetupAnswers: {
        names: 'Alex & Jordan',
  ...
  it('keeps closed legacy string indexes aligned with the stored setup progress boundary', () => {
    const normalized = normalizeQuickStartDraftSnapshot({
      currentIndex: '0',
      initialSetupAnswers: {
        names: 'Alex & Jordan',
  ...
  it('clamps follow-up restore progress to the setup question count when stale indexes overshoot', () => {
    const normalized = normalizeQuickStartDraftSnapshot({
      currentIndex: 99,
      initialSetupAnswers: {
        names: 'Alex & Jordan',
  ...
  it('restores draft-only clarifying snapshots when older payloads omitted questions and history', () => {
    const normalized = normalizeQuickStartDraftSnapshot({
      clarifyingState: {
        clarifying: {
          mode: 'draft',
