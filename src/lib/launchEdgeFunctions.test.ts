...
describe('launch edge function guards', () => {
  ...
  it('keeps registry preview product fetching public-only and customer-safe', () => {
    ...
    expect(source).toContain('function isPrivateIpv6');
    expect(source).toContain("import { normalizeUrl, isSameProduct, isPublicPreviewResourceUrl } from './urlNormalizer.ts'");
    ...
    expect(urlNormalizer).toContain('export function isPublicPreviewResourceUrl(url: string, depth = 0)');
    expect(urlNormalizer).toContain("parsed.protocol === 'http:' || parsed.protocol === 'https:'");
    expect(urlNormalizer).toContain('!parsed.username');
    expect(urlNormalizer).toContain('!parsed.password');
    expect(urlNormalizer).toContain('!isBlockedHostname(parsed.hostname)');
    expect(urlNormalizer).toContain("parsed.hostname.toLowerCase() === 'images.weserv.nl'");
    expect(urlNormalizer).toContain("const proxiedTarget = parsed.searchParams.get('url')");
    expect(urlNormalizer).toContain('if (depth >= 2) return false');
    expect(urlNormalizer).toContain('return isPublicPreviewResourceUrl(normalizedTarget, depth + 1)');
  ...
  it('keeps guest photo upload readiness clean and hides raw backend failures from guests', () => {
    ...
    expect(source).toContain('if (guestEmail && !/^[^\\s@<>"\'()]+@[^\\s@<>"\'()]+\\.[^\\s@<>"\'()]+$/.test(guestEmail))');
    ...
    expect(source).not.toContain('/^[^\\s@]+@[^\\s@]+\\.[^\\s@]+$/');
