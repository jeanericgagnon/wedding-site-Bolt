...
describe('launch edge function guards', () => {
  ...
  it('keeps public guest contact updates session-scoped instead of browser-id scoped', () => {
    ...
    expect(contactPage).toContain('const guestIdentity = buildGuestContactIdentityPayload(siteRef, searchParams);');
    expect(contactPage).toContain('...guestIdentity');
    expect(contactPage).toContain("apply_household: false");
    const contactLookupPanel = readFileSync(join(process.cwd(), 'src', 'pages', 'GuestContactLookupPanel.tsx'), 'utf8');
    expect(contactLookupPanel).toContain('placeholder="Search your full name"');
    expect(contactLookupPanel).toContain('placeholder="First few letters of your email"');
    expect(contactLookupPanel).toContain('If you opened this from your invitation link, we can use that to help find your record too.');
    expect(contactPage).toContain('...buildGuestContactAccessPayload(siteRef, searchParams)');
  ...
  it('keeps public guest Edge Function readiness and raw-error contracts in prereqs', () => {
    ...
    expect(vaultSubmit).toContain('.select("id,site_slug,is_published,privacy_mode,guest_access_token,wedding_date,user_id")');
