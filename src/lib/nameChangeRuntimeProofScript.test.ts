...
describe('name change runtime proof script', () => {
  it('supports a dedicated live runtime smoke for the name-change planner route', () => {
    ...
    expect(source).toContain('src/lib/nameChange/requirements.test.ts');
    expect(source).toContain('src/lib/nameChange/reminders.test.ts');
    expect(source).toContain('src/lib/nameChange/tsaFlow.test.ts');
    expect(source).toContain('src/lib/nameChange/dmvFlow.test.ts');
