...
describe("photo-upload-moderate safety", () => {
  it("keeps lookup, update, and unexpected failures admin-safe", () => {
    ...
    expect(functionSource).toContain('console.error("PHOTO_UPLOAD_MODERATE_LOOKUP_FAILED"');
    expect(functionSource).toContain('reason: "PHOTO_UPLOAD_MODERATION_LOOKUP_ERROR"');
    expect(functionSource).toContain('console.error("PHOTO_UPLOAD_MODERATE_PARTIAL_LOOKUP_FAILED"');
    expect(functionSource).toContain('reason: "PHOTO_UPLOAD_MODERATION_PARTIAL_LOOKUP"');
    expect(functionSource).toContain('console.error("PHOTO_UPLOAD_MODERATE_SITE_LOOKUP_FAILED"');
    expect(functionSource).toContain('reason: "PHOTO_UPLOAD_MODERATION_SITE_LOOKUP_ERROR"');
    ...
    expect(functionSource).not.toContain('fail("DB_ERROR", uploadsErr?.message ?? "No uploads found", 400)');
