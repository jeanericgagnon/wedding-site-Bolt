...
describe('proof board freshness', () => {
  it('derives the board from the current backlog truth', () => {
    ...
    expect(board.summary?.currentProofState).toContain('test:smoke');
    expect(board.summary?.currentNextActions ?? '').toContain('Commit/push this blocker-fix batch');
    expect(board.ruthlessNextThree?.some((item) => item.title?.includes('Commit and push'))).toBe(true);
    expect(board.ruthlessNextThree?.some((item) => item.title?.includes('Deploy the frontend billing-gate fix'))).toBe(true);
    expect(board.currentState?.['Reason production-ready is not yet claimed']).toContain('locally proven');
    expect(board.sections?.['Current Canonical Status']).toContain('| Current launch verdict | `HOLD` |');
    expect(board.sections?.['Current Canonical Status']).toContain('| Production-ready | `NO` |');
