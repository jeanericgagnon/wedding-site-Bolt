      ...
      <div className="max-w-6xl mx-auto px-5 md:px-10">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-5 gap-4 md:gap-6">
          <div className="lg:col-span-2 rounded-2xl border border-border/40 bg-white p-4 md:p-5">
            <p className="text-sm text-text-tertiary mb-3">Map preview</p>
            <div className="rounded-xl border border-dashed border-border/60 bg-surface-subtle h-64 flex items-center justify-center text-center px-4">
              <div>
                ...
                <p className="text-sm text-text-secondary">Guests can use the location links above for directions.</p>
              </div>
            </div>
          </div>

          <div className="lg:col-span-3 rounded-2xl border border-border/40 bg-white p-4 md:p-5">
            <p className="text-sm text-text-tertiary mb-3">Pinned locations</p>
            <div className="space-y-2.5">
              {data.pins.length === 0 ? (
                <p className="text-xs text-text-tertiary">No pinned locations yet.</p>
              ) : data.pins.map((pin) => {
                const safePinUrl = getSafePublicWebUrl(pin.url);
                return (
                <div key={pin.id} className="rounded-xl border border-border/35 bg-surface px-3 py-2.5">
                  <div className="flex items-center justify-between gap-2">
                    ...
                    {safePinUrl ? (
                      <a href={safePinUrl} target="_blank" rel="noopener noreferrer" className="text-text-tertiary hover:text-primary" aria-label={`Open ${pin.name} location link`}>
