...
describe('sections registry resolution', () => {
  ...
  it('keeps runtime registry resolution tolerant of persisted trim casing and punctuation drift', () => {
    ...
    expect(registryLinkCarryover).toContain("const existingMode = (existing as CarryoverRegistryLink & { sourceLabelMode?: 'explicit' | 'inferred' }).sourceLabelMode;");
  ...
  it('keeps public purchaser status aligned with owner purchase state truth', () => {
    ...
    const registryDataHook = readFileSync(resolve(__dirname, '../pages/dashboard/registry/useRegistryDashboardData.ts'), 'utf8');
    const registryItemActions = readFileSync(resolve(__dirname, '../pages/dashboard/registry/useRegistryItemActions.ts'), 'utf8');
    const registryMaintenanceActions = readFileSync(resolve(__dirname, '../pages/dashboard/registry/useRegistryMaintenanceActions.ts'), 'utf8');
    ...
    expect(registryDataHook).toContain('export function normalizeOwnerDashboardRegistryItem(item: RegistryItem): RegistryItem {');
    expect(registryDataHook).toContain('setItems(data.map(normalizeOwnerDashboardRegistryItem));');
    expect(registryDerivedState).toContain('const duplicateGroups = findDuplicateRegistryGroups(items);');
    expect(registryDerivedState).toContain('const actionableBadImportCount = items.filter((item) => getRegistryItemMetadataState(item).hasBadImportTitle && !!(item.item_url || item.canonical_url)).length;');
    expect(registryItemActions).toContain('normalizeOwnerDashboardRegistryItem({ ...item, ...fields, updated_at: new Date().toISOString() })');
    expect(registryItemActions).toContain('normalizeOwnerDashboardRegistryItem(updated)');
    expect(registryItemActions).toContain('normalizeOwnerDashboardRegistryItem(created)');
    expect(registryDerivedState).toContain('const filtered = items.filter((item) => {');
    expect(registryDataHook).toContain('purchaser_name: quantityState.purchaseStatus === \'available\' ? null : item.purchaser_name,');
    expect(registryDerivedState).toContain('badImports: items.filter((item) => getRegistryItemMetadataState(item).hasBadImportTitle).length,');
    expect(registryDerivedState).toContain('repair: actionableBadImportCount,');
    expect(registryDerivedState).toContain('items.filter((item) => getRegistryItemMetadataState(item).hasBadImportTitle).length,');
    expect(registryDerivedState).toContain("imageIssues: items.filter((item) => !item.image_url || item.image_url.includes('thum.io') || item.image_url.includes('weserv.nl')).length,");
    expect(registryMaintenanceActions).toContain('normalizeOwnerDashboardRegistryItem(updated)');
  });
