...
describe('getRegistryPurchaseCtaLabel', () => {
  ...
  it('keeps contradictory purchased items from disappearing into false empty-state copy', () => {
    const normalized = normalizePublicRegistryItemState({
      id: 'item-1',
      wedding_site_id: 'site-1',
      item_name: 'Mixer',
      price_label: null,
    ...
    expect(getRegistryEmptyStateMessage([normalized], 'all')).toBe('');
  });
});
