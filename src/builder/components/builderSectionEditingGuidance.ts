...
export function getBuilderSectionEditingGuidance({
  ...
  if (!enabled) {
    return {
      ...
      focusDetail: 'The section still exists in the draft, but none of its content matters until you decide whether it belongs back on the visible page.',
      ...
      watchout: 'Hidden sections are easy to forget, which quietly creates two versions of the page: the visible one and the abandoned draft one.',
  ...
  if (hasBindings && !dataConfigured) {
    return {
      ...
      currentStep: 'Bind the real data this section is supposed to represent.',
  ...
  return {
    ...
    decisionRule: 'Once a section is doing its job, the next move is usually to verify it in the full visible page flow, not to keep changing it in isolation.',
