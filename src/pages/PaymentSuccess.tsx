    ...
    const run = async () => {
      ...
      if (sessionId) {
        try {
          ...
          if (verified.paid) {
            setStatus('confirmed');
            setTimeout(() => navigate('/onboarding?from=checkout&showChooser=1', { replace: true }), 1200);
            return;
          }
        } catch {
      ...
      const poll = async () => {
        try {
          ...
          if (paymentStatus === 'active') {
            setStatus('confirmed');
            setTimeout(() => navigate('/onboarding?from=checkout&showChooser=1', { replace: true }), 1500);
            return;
          }
        } catch {
          // silent — keep polling
