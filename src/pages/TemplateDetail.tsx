...
export const TemplateDetail: React.FC = () => {
  ...
  return (
    <div className="min-h-screen bg-neutral-50 px-4 py-10">
      <div className="mx-auto max-w-5xl rounded-lg border border-neutral-200 bg-white overflow-hidden">
        <img src={tpl.previewImage} alt={tpl.name} className="h-56 md:h-72 w-full object-cover" />
        <div className="p-6">
          <div className="mb-4 flex flex-wrap gap-2 text-xs">
            <span className="rounded-lg border border-primary/20 bg-primary/5 px-2.5 py-1 text-primary">Core wedding pages included</span>
            <span className="rounded-lg border border-neutral-200 bg-white px-2.5 py-1 text-neutral-700">Easy to personalize</span>
            <span className="rounded-lg border border-neutral-200 bg-white px-2.5 py-1 text-neutral-700">Share when you are ready</span>
          </div>
            ...
            {supportManifest && (
              <div className="rounded-lg border border-neutral-200 bg-neutral-50 p-3 md:col-span-3">
                <div className="flex flex-wrap items-center gap-2">
                  ...
                  <span className={`rounded-lg border px-2 py-0.5 text-[11px] font-medium ${supportManifest.previewStatus === 'verified' ? 'border-primary/20 bg-primary/5 text-primary' : 'border-neutral-200 bg-white text-neutral-700'}`}>{supportManifest.previewLabel}</span>
                  <span className="rounded-lg border border-neutral-200 bg-white px-2 py-0.5 text-[11px] text-neutral-700">{supportManifest.sectionsIncluded} starter sections</span>
                  <span className="rounded-lg border border-neutral-200 bg-white px-2 py-0.5 text-[11px] text-neutral-700">{supportManifest.modulesIncluded} features</span>
                </div>
            ...
            <div className="mt-6">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                {relatedTemplates.map((rel) => {
                  ...
                  return (
                    <div key={rel.id} className="rounded-lg border border-neutral-200 bg-white p-2 hover:border-primary/35">
      ...
      <div className="fixed bottom-3 left-3 right-3 md:hidden z-20">
        <button onClick={() => handleUseTemplate()} className="w-full rounded-lg bg-primary px-4 py-3 text-sm font-semibold text-white shadow-md hover:bg-primary-hover">Use this design</button>
      </div>
    </div>
  );
};
