...
describe('MessageDetailModal', () => {
  it('shows focused retry and exclusion controls for reviewed rows', async () => {
    ...
    expect(screen.getByText('1 of 4 targeted recipients currently need review.')).toBeInTheDocument();
    expect(screen.getByText('1 of 4 targeted recipients still need contact details.')).toBeInTheDocument();
  ...
  it('keeps cleanup closure explicit when a send is fully closed out', () => {
    render(
      <MessageDetailModal
        message={message({
          recipient_count: 3,
