            ...
            {checkInQuery.trim() && canEditQna && (
              <div className="rounded-lg border border-primary/20 bg-accent-light px-3 py-3">
                ...
                <div className="mt-2 flex flex-wrap gap-2">
                  <button type="button" onClick={() => onRouteNoMatch('walk-in')} className="rounded-md border border-primary/20 bg-white px-2.5 py-1.5 text-[11px] text-primary">Route walk-in</button>
                  <button type="button" onClick={() => onRouteNoMatch('help-desk')} className="rounded-md border border-primary/20 bg-white px-2.5 py-1.5 text-[11px] text-primary">Send to help desk</button>
                  <button type="button" onClick={() => onRouteNoMatch('manager-decision')} className="rounded-md border border-primary/20 bg-white px-2.5 py-1.5 text-[11px] text-primary">Ask manager</button>
                </div>
              </div>
            )}
          </div>
              ...
              <div>
                <div className="flex items-center gap-2 flex-wrap">
                  ...
                  {activeGuestId === guest.id && checkInBoardTargetId !== guest.id && (
                    <span className="px-2 py-0.5 rounded-lg text-[10px] border border-primary/20 bg-primary/5 text-primary">Selected guest</span>
                  )}
                </div>
                ...
                {visibleExceptionStates.length > 0 && (
                  <div className="mt-1 flex flex-wrap gap-1">
                    {visibleExceptionStates.map((state) => (
                      <span key={`${guest.id}-${state}`} className="rounded-md border border-primary/15 bg-primary/5 px-2 py-0.5 text-[10px] text-primary">
    ...
    <div>
      ...
      <div className="mb-2 rounded-lg border border-border/50 bg-surface-subtle/25 px-3 py-3">
        <div className="flex items-start justify-between gap-3">
          ...
          <span className={`rounded-lg border px-2 py-1 text-[10px] font-medium ${boardToneClassName(timelineBoard.tone)}`}>
          ...
          <div className="flex flex-col gap-2 md:flex-row md:items-center md:justify-between">
            ...
            <div className="flex flex-wrap gap-2">
              {activeTimelineCorrectionAction && (
                <button type="button" disabled={!canEditTimeline} onClick={() => onRunAction(activeTimelineEvent.id, activeTimelineCorrectionAction.nextState)} className="rounded border border-primary/20 bg-accent-light px-2.5 py-1 text-[11px] text-primary disabled:opacity-40">
                  {activeTimelineCorrectionAction.label}
                </button>
              )}
              {activeTimelinePrimaryAction?.nextState && (
                <button type="button" disabled={!canEditTimeline} onClick={() => onRunAction(activeTimelineEvent.id, activeTimelinePrimaryAction.nextState)} className="text-[11px] px-2.5 py-1 rounded border border-primary/25 bg-primary/5 text-primary disabled:opacity-40">
      ...
      {(liveEventId || upNextEventId || timelineBoardTargetId) && (
        <div className="mb-2 flex flex-wrap gap-1.5">
          {liveEventId && <button type="button" onClick={() => onJumpToEvent(liveEventId)} className={`text-[11px] px-2 py-1 rounded-lg border ${activeTimelineEventId === liveEventId ? 'border-primary/30 bg-primary/10 text-primary' : 'border-primary/20 bg-primary/5 text-primary hover:bg-primary/10'}`}>Jump to live event</button>}
          {upNextEventId && <button type="button" onClick={() => onJumpToEvent(upNextEventId)} className={`rounded-lg border px-2 py-1 text-[11px] ${activeTimelineEventId === upNextEventId ? 'border-primary/20 bg-accent-light text-primary' : 'border-border-subtle bg-surface-subtle text-text-secondary hover:border-primary/35 hover:text-primary'}`}>Jump to up next</button>}
          {timelineBoardTargetId && timelineBoardTargetId !== liveEventId && timelineBoardTargetId !== upNextEventId && <button type="button" onClick={() => onJumpToEvent(timelineBoardTargetId)} className={`text-[11px] px-2 py-1 rounded-lg border ${activeTimelineEventId === timelineBoardTargetId ? 'border-primary/30 bg-primary/10 text-primary' : 'border-border bg-white text-text-secondary hover:border-primary/35 hover:text-primary'}`}>Jump to suggested event</button>}
        </div>
      )}
      <div className="space-y-2">
        {events.length === 0 ? (
          ...
          events.map((event) => {
            ...
            return (
              <div key={event.id} className={`cursor-pointer rounded-lg border px-3 py-2 ${activeTimelineEventId === event.id ? 'ring-2 ring-primary/10 ' : ''}${isLive ? 'border-primary/35 bg-primary/5' : isUpNext ? 'border-primary/20 bg-accent-light' : 'border-border/50 bg-surface-subtle/40'}`} onClick={() => { onFocusLane(); onSelectEvent(event.id); }}>
                <div className="flex items-center justify-between gap-2">
                  <div>
                    <div className="flex flex-wrap items-center gap-2">
                      ...
                      {activeTimelineEventId === event.id && timelineBoardTargetId !== event.id && <span className="px-2 py-0.5 rounded-lg text-[10px] border border-primary/20 bg-primary/5 text-primary">Selected event</span>}
                    </div>
                    <p className="text-[11px] text-text-tertiary">{isLive ? 'Live now' : isUpNext ? 'Up next' : state === 'done' ? 'Completed' : 'Queued'}</p>
                  </div>
                  <select value={state} onClick={(event) => event.stopPropagation()} onChange={(selectEvent) => { if (canEditTimeline) onSelectState(event.id, selectEvent.target.value as TimelineState); }} disabled={!canEditTimeline} className="text-[11px] rounded-md border border-border bg-white px-2 py-1 text-text-secondary disabled:opacity-40">
                ...
                <div className="mt-2 flex items-center justify-between gap-3">
                  ...
                  <div className="flex items-center gap-2">
                    {correctionAction && <button type="button" disabled={!canEditTimeline} onClick={(clickEvent) => { clickEvent.stopPropagation(); onRunAction(event.id, correctionAction.nextState); }} className="rounded border border-primary/20 bg-accent-light px-2.5 py-1 text-[11px] text-primary disabled:opacity-40">{correctionAction.label}</button>}
                    <button type="button" disabled={!canEditTimeline || !primaryAction.nextState} onClick={(clickEvent) => { clickEvent.stopPropagation(); onRunAction(event.id, primaryAction.nextState); }} className="text-[11px] px-2.5 py-1 rounded border border-border bg-white text-text-secondary disabled:opacity-40">{primaryAction.label}</button>
                  </div>
                </div>
              </div>
            );
    ...
    <div className="border-t border-border/60 pt-3">
      ...
      <div className="mb-3 rounded-lg border border-border/50 bg-surface-subtle/25 px-3 py-3">
        <div className="flex items-start justify-between gap-3">
          ...
          <span className={`rounded-lg border px-2 py-1 text-[10px] font-medium ${alertBoard.statusTone === 'ready' ? 'border-primary/20 bg-primary/5 text-primary' : alertBoard.statusTone === 'warning' ? 'border-primary/20 bg-accent-light text-primary' : 'border-border bg-white text-text-tertiary'}`}>{alertBoard.statusLabel}</span>
        </div>
        <div className="mt-3 grid grid-cols-1 gap-2 md:grid-cols-2">
          <div className="rounded-md border border-border/50 bg-white px-2.5 py-2"><p className="text-[10px] text-text-tertiary">Delivery</p><p className="mt-1 text-[11px] text-text-primary">{alertBoard.deliveryLabel}</p></div>
          <div className="rounded-md border border-border/50 bg-white px-2.5 py-2"><p className="text-[10px] text-text-tertiary">Latest activity</p><p className="mt-1 text-[11px] text-text-primary">{alertBoard.latestActivityLabel}</p></div>
        </div>
      </div>
      <div className="mb-3 rounded-lg border border-border/50 bg-surface-subtle/25 px-3 py-3">
        <div className="flex items-start justify-between gap-3">
          ...
          <span className={`rounded-lg border px-2 py-1 text-[10px] font-medium ${alertActivityBoard.tone === 'ready' ? 'border-primary/20 bg-primary/5 text-primary' : alertActivityBoard.tone === 'warning' ? 'border-primary/20 bg-accent-light text-primary' : 'border-border bg-white text-text-tertiary'}`}>{alertActivityBoard.statusLabel}</span>
        </div>
        <div className="mt-3 grid grid-cols-1 gap-2 md:grid-cols-2">
          <div className="rounded-md border border-border/50 bg-white px-2.5 py-2"><p className="text-[10px] text-text-tertiary">Channel mix</p><p className="mt-1 text-[11px] text-text-primary">{alertActivityBoard.channelLabel}</p></div>
          <div className="rounded-md border border-border/50 bg-white px-2.5 py-2"><p className="text-[10px] text-text-tertiary">Pacing</p><p className="mt-1 text-[11px] text-text-primary">{alertActivityBoard.pacingLabel}</p></div>
        </div>
      </div>
      <div className="flex flex-wrap gap-1.5 mb-3">
        {alertSuggestions.map((suggestion) => {
          ...
          return (
            <button key={suggestion.key} type="button" disabled={!canSendAlerts} onClick={() => { onFocusLane(); onSetAlertForm((prev) => ({ ...prev, subject: suggestion.subject, body: suggestion.body, audience: suggestion.audience })); onSetLastAlertSuggestionKey(suggestion.key); }} className={`text-[11px] px-2 py-1 rounded-lg border inline-flex items-center gap-1.5 disabled:opacity-40 ${suggestionState.isDraftMatch ? 'border-primary/35 bg-primary/10 text-primary' : suggestionState.isBoardTarget ? 'border-primary/25 bg-primary/5 text-primary hover:bg-primary/10' : 'border-border bg-white text-text-secondary hover:border-primary/40 hover:text-primary'}`}>
              <span>{suggestion.label}</span>
              {suggestionState.badge && <span className={`px-1.5 py-0.5 rounded-lg border text-[9px] font-medium ${suggestionState.isDraftMatch ? 'border-primary/25 bg-white/80 text-primary' : 'border-primary/15 bg-primary/[0.04] text-primary/80'}`}>{suggestionState.badge}</span>}
            </button>
          );
        })}
        <button type="button" disabled={!canSendAlerts} onClick={() => { onFocusLane(); onSetAlertForm((prev) => ({ ...prev, channel: 'sms', scheduleType: 'now' })); }} className="text-[11px] px-2 py-1 rounded-lg border border-border bg-white text-text-secondary hover:border-primary/40 hover:text-primary disabled:opacity-40">Text now</button>
        <button type="button" disabled={!canSendAlerts || !canScheduleAlerts} onClick={() => { onFocusLane(); onSetAlertForm((prev) => ({ ...prev, channel: 'email', scheduleType: 'later' })); }} className="text-[11px] px-2 py-1 rounded-lg border border-border bg-white text-text-secondary hover:border-primary/40 hover:text-primary disabled:opacity-40">Schedule email</button>
        {alertStats.byAudience.map(([audience, count]) => (
          <button key={audience} type="button" disabled={!canSendAlerts} onClick={() => { onFocusLane(); onSetAlertForm((prev) => ({ ...prev, audience })); }} className="text-[11px] px-2 py-1 rounded-lg border border-border bg-white text-text-secondary hover:border-primary/40 hover:text-primary disabled:opacity-40">{audience} ({count})</button>
        ))}
      </div>
      <fieldset disabled={!canSendAlerts} className="space-y-2.5">
        ...
        <div className="grid grid-cols-2 gap-2">
          <select value={alertForm.audience} onChange={(event) => { onFocusLane(); onSetAlertForm((prev) => ({ ...prev, audience: event.target.value })); }} className="text-xs rounded-md border border-border bg-white px-2 py-2 text-text-secondary">
            <option value="all">All guests</option>
          ...
          <select value={alertForm.channel} onChange={(event) => { onFocusLane(); onSetAlertForm((prev) => ({ ...prev, channel: event.target.value as 'email' | 'sms' })); }} className="text-xs rounded-md border border-border bg-white px-2 py-2 text-text-secondary">
            <option value="email">Email</option>
            <option value="sms">Text</option>
          </select>
        </div>
        <div className="grid grid-cols-2 gap-2">
          <select value={alertForm.scheduleType} onChange={(event) => { onFocusLane(); onSetAlertForm((prev) => ({ ...prev, scheduleType: (canScheduleAlerts ? event.target.value : 'now') as 'now' | 'later' })); }} className="text-xs rounded-md border border-border bg-white px-2 py-2 text-text-secondary">
            <option value="now">Send now</option>
            <option value="later" disabled={!canScheduleAlerts}>Schedule</option>
          </select>
          {alertForm.scheduleType === 'later' && canScheduleAlerts ? (
            <div className="grid grid-cols-2 gap-2">
              <input type="date" value={alertForm.scheduleDate} onChange={(event) => { onFocusLane(); onSetAlertForm((prev) => ({ ...prev, scheduleDate: event.target.value })); }} className="text-xs rounded-md border border-border bg-white px-2 py-2 text-text-secondary" />
              <input type="time" value={alertForm.scheduleTime} onChange={(event) => { onFocusLane(); onSetAlertForm((prev) => ({ ...prev, scheduleTime: event.target.value })); }} className="text-xs rounded-md border border-border bg-white px-2 py-2 text-text-secondary" />
            </div>
          ) : <div />}
        </div>
          ...
          {!alertTargetCue.aligned && (
            <div className="space-y-2">
              ...
              <div className="flex flex-wrap items-center gap-2 text-[11px]">
                ...
                {preferredAlertSuggestion && <button type="button" onClick={() => { onFocusLane(); onSetAlertForm((prev) => applyCoordinatorAlertSuggestion({ form: prev, suggestion: preferredAlertSuggestion })); }} className="inline-flex w-fit rounded-md border border-border-subtle bg-white px-2.5 py-1 text-[11px] font-medium text-primary">Re-align to {preferredAlertSuggestion.label.toLowerCase()}</button>}
              </div>
            </div>
          )}
        ...
        <button type="button" onClick={onSendAlert} disabled={alertBusy || !!alertValidationError || !canSendAlerts} className="w-full px-3 py-2 text-sm rounded-md border border-primary/30 bg-primary/10 text-primary disabled:opacity-50">
        ...
        {commandSummaryItems.map((item) => (
          <button key={item.label} type="button" onClick={() => onCommandClick(item.label)} className={`rounded-xl border px-3 py-2.5 text-left transition hover:border-primary/35 hover:bg-primary/[0.04] ${item.tone === 'priority' ? 'border-primary/30 bg-primary/[0.06]' : item.tone === 'ready' ? 'border-border-subtle bg-surface-subtle' : 'border-border/50 bg-surface-subtle/35'}`}>
            <div className="flex items-start justify-between gap-2">
              ...
              <span className={`rounded-lg border px-1.5 py-0.5 text-[9px] font-medium ${item.tone === 'priority' ? 'border-primary/20 bg-white text-primary' : item.tone === 'ready' ? 'border-border-subtle bg-white text-primary' : 'border-border bg-white text-text-tertiary'}`}>{item.statusLabel}</span>
            </div>
            ...
            <div className="mt-2 inline-flex items-center gap-1 rounded-lg border border-border/60 bg-white/80 px-2 py-1 text-[9px] font-medium text-text-secondary"><span className="text-text-tertiary">Next</span><span>{item.actionLabel}</span></div>
            {priorityCommandLabel === item.label && <div className="mt-2 inline-flex flex-wrap items-center gap-1 rounded-lg border border-primary/20 bg-white/80 px-2 py-1 text-[9px] font-medium text-primary"><span>Now — {priorityCommandReason}{priorityCommandTargetReason ? ` ${priorityCommandTargetReason}` : ''}</span><span className="rounded-lg border border-primary/15 bg-primary/[0.05] px-1.5 py-0.5">{priorityCommandCta}</span></div>}
          </button>
        ))}
      </div>
      <div className="mt-3 grid grid-cols-1 gap-2 md:grid-cols-2 xl:grid-cols-4">
        {commandDeckItems.map((item) => (
          <button key={item.label} type="button" onClick={() => onCommandClick(item.label)} className={`rounded-xl border px-3 py-3 text-left transition hover:border-primary/35 hover:bg-primary/[0.04] ${item.priority ? 'border-primary/30 bg-primary/[0.06]' : 'border-border/50 bg-surface-subtle/25'}`}>
            <div className="flex items-start justify-between gap-2">
              <p className="text-[11px] font-medium text-text-primary">{item.label}</p>
              <span className={`rounded-lg border px-1.5 py-0.5 text-[9px] font-medium ${item.priority ? 'border-primary/20 bg-white text-primary' : 'border-border bg-white text-text-tertiary'}`}>{item.status}</span>
            </div>
        ...
        {opsSnapshotItems.map((item) => (
          <button key={item.key} type="button" onClick={() => onOpsSnapshotClick(item.key)} className={`rounded-xl border px-3 py-3 text-left transition hover:border-primary/35 hover:bg-primary/[0.04] ${item.tone === 'warning' ? 'border-primary/20 bg-accent-light' : item.tone === 'success' ? 'border-border-subtle bg-surface-subtle' : 'border-border/50 bg-surface-subtle/30'}`}>
            <div className="flex items-start justify-between gap-2">
              ...
              <span className={`rounded-lg border px-1.5 py-0.5 text-[9px] font-medium ${item.locked ? 'border-border bg-white text-text-tertiary' : item.tone === 'warning' ? 'border-border-subtle bg-white text-primary' : item.tone === 'success' ? 'border-border-subtle bg-white text-primary' : 'border-primary/20 bg-white text-primary'}`}>{item.locked ? 'Read only' : item.tone === 'warning' ? 'Needs action' : item.tone === 'success' ? 'On track' : 'Ready'}</span>
            </div>
            <p className="mt-3 text-[10px] font-medium text-text-tertiary">{item.cta}</p>
          </button>
        ))}
