...
export function SeatingDashboardRouteContent(props: {
  ...
  return (
    <div className="space-y-5" onClick={() => props.setSelectedTableId(null)}>
      <DashboardPageHero
        eyebrow="Seating"
        ...
        actions={
          <>
            <Link to="/dashboard/seating-lookup" className="rounded-lg border border-border-subtle bg-surface px-3 py-2 text-sm font-medium text-text-primary no-underline hover:bg-surface-subtle">Open seating lookup</Link>
            <Link to="/dashboard/coordinator" className="rounded-lg border border-border-subtle bg-surface px-3 py-2 text-sm font-medium text-text-primary no-underline hover:bg-surface-subtle">Wedding Day</Link>
          </>
        }
      >
        <div className="flex flex-wrap gap-2 items-center">
          ...
          <div className="inline-flex rounded-lg border border-border bg-surface-subtle p-0.5">
            <button
              className={`rounded-lg px-4 py-2 text-sm transition-colors ${props.layoutMode === 'visual' ? 'border border-primary/25 bg-primary/10 text-primary' : 'text-text-tertiary hover:text-text-primary'}`}
              onClick={() => props.setLayoutMode('visual')}
            >
              Canvas Layout
            </button>
            <button
              className={`rounded-lg px-4 py-2 text-sm transition-colors ${props.layoutMode === 'list' ? 'border border-primary/25 bg-primary/10 text-primary' : 'text-text-tertiary hover:text-text-primary'}`}
              onClick={() => props.setLayoutMode('list')}
            >
              List Layout
            </button>
          </div>
          <div className="rounded-lg border border-border-subtle bg-surface px-3 py-2 text-xs text-text-secondary">
      ...
      <section className="rounded-[20px] border border-border-subtle bg-surface p-5 shadow-none">
        <div className="flex flex-col gap-3 lg:flex-row lg:items-end lg:justify-between">
          ...
          <div className="inline-flex flex-wrap gap-2 text-xs text-text-tertiary">
            <span className="rounded-lg border border-border-subtle bg-surface-subtle/30 px-3 py-1">Event-specific layouts</span>
            <span className="rounded-lg border border-border-subtle bg-surface-subtle/30 px-3 py-1">Packet-ready exports</span>
            <span className="rounded-lg border border-border-subtle bg-surface-subtle/30 px-3 py-1">Lookup and check-in nearby</span>
          </div>
        </div>
      </section>
      ...
      <div className={`rounded-[20px] border p-4 shadow-none ${props.packetReadyTone}`}>
        ...
        <div className="mt-3 rounded-[20px] border border-border-subtle bg-surface p-3">
          ...
          {props.cateringHandoffReview.warnings.length > 0 && (
            <div className="mt-3 rounded-[20px] border border-primary/20 bg-primary/5 px-3 py-2">
      ...
      <details className="rounded-[20px] border border-border-subtle bg-surface-subtle/40 p-3">
        ...
        <div className="mt-3 space-y-3">
          {props.layoutMode === 'visual' && (
            <div className="hidden flex-wrap items-center gap-3 rounded-[20px] border border-border-subtle bg-surface-subtle px-3 py-2 text-xs text-text-tertiary sm:flex">
          ...
          {props.invalidCount > 0 && (
            <div className="flex items-center gap-2 rounded-[20px] border border-error/20 bg-error/5 p-3 text-sm">
      ...
      <div className="rounded-[20px] border border-border-subtle bg-surface-subtle/40 p-3">
        ...
        <div className="grid gap-2 sm:grid-cols-3">
          <button
            ...
            className="rounded-[20px] border border-border-subtle bg-surface px-4 py-4 text-left hover:border-primary/25 hover:bg-primary-light/10 disabled:opacity-50"
          >
          ...
          <button
            ...
            className="rounded-[20px] border border-border-subtle bg-surface px-4 py-4 text-left hover:border-primary/25 hover:bg-surface-subtle/60 disabled:opacity-50"
          >
        ...
        <div className="space-y-3 rounded-[20px] border border-border-subtle bg-surface-subtle/40 p-4">
          ...
          <div className="flex flex-wrap items-center gap-2">
            <input
              ...
              className="min-w-[220px] flex-1 rounded-lg border border-border bg-surface px-3 py-2 text-sm text-text-primary focus:outline-none focus:ring-2 focus:ring-primary"
            />
            <select
          ...
          <div className="flex flex-wrap items-center gap-2 text-xs text-text-tertiary">
            ...
            {props.checkInCandidates.some((guest) => !props.arrivedGuestIds.has(guest.id)) && (
              <button
                ...
                className="rounded-lg border border-primary/25 bg-primary/10 px-3 py-1 text-primary hover:bg-primary/15"
              >
            ...
            {props.checkInCandidates.some((guest) => props.arrivedGuestIds.has(guest.id)) && (
              <button
                ...
                className="rounded-lg border border-border-subtle bg-surface px-3 py-1 text-text-secondary hover:border-primary/30 hover:text-primary"
              >
              ...
              {props.checkInCandidates.length === 0 ? (
                ...
                return (
                  <button
                    ...
                    className={`rounded-lg border px-2.5 py-1.5 text-xs transition-colors ${checked ? 'border-primary/30 bg-primary/10 text-primary' : 'border-border-subtle bg-surface text-text-secondary hover:border-primary/40 hover:text-primary'}`}
                  >
      ...
      {props.showAutoTablesModal && (
        <div className="space-y-3 rounded-[20px] border border-border-subtle bg-surface-subtle p-4">
          ...
          <div className="flex items-center gap-3">
            ...
            <input
              ...
              className="w-20 rounded-lg border border-border bg-surface px-2.5 py-1.5 text-sm text-text-primary focus:outline-none focus:ring-2 focus:ring-primary"
            />
      ...
      {props.showResetConfirm && (
        <div className="flex items-start justify-between gap-4 rounded-[20px] border border-error/20 bg-error/5 p-4">
      ...
      {props.seatPicker && (
        <div className="space-y-4 rounded-[20px] border border-border-subtle bg-surface-subtle/40 p-4">
          ...
          {props.activeSeatGuest && (
            <div className="flex items-center justify-between gap-3 rounded-[20px] border border-primary/20 bg-primary-light/20 px-3 py-3">
              <div>
          ...
          <div className="grid gap-2 sm:grid-cols-2 lg:grid-cols-3">
            {props.seatPickerOptions.slice(0, 18).map((guest) => (
              <button
                ...
                className="rounded-lg border border-border bg-surface px-3 py-2 text-left hover:border-primary/40 hover:bg-primary-light/20"
              >
          ...
          {props.seatPickerOptions.length === 0 && (
            <div className="rounded-lg border border-border bg-surface px-3 py-4 text-sm text-text-tertiary">
        ...
        <DndContext sensors={props.sensors} onDragStart={props.handleDragStart} onDragEnd={props.handleDragEnd}>
          <div className="flex flex-col lg:flex-row gap-5">
            <div className="lg:w-64 xl:w-72 flex-shrink-0">
              <div className="sticky top-24 space-y-3 rounded-[20px] border border-border-subtle bg-surface-subtle/40 p-3">
            ...
            <div className="flex-1 min-w-0">
              {props.layoutMode === 'visual' && (
                <div className="mb-3 flex items-center justify-between gap-2 rounded-[20px] border border-border-subtle bg-surface-subtle p-2 text-xs text-text-tertiary">
              ...
              {props.tables.length === 0 ? (
                <div className="rounded-[20px] border-2 border-dashed border-border-subtle py-16 text-center">
                ...
                <>
                  ...
                  <div
                    className={`relative min-h-[720px] overflow-auto rounded-[20px] border border-border-subtle bg-surface transition-all duration-300 ${props.canvasFullscreen ? 'bg-white p-3' : ''}`}
                  ...
                  >
                    {props.canvasFullscreen && (
                      <div className="mb-2 flex items-center justify-between animate-in fade-in duration-200">
                        <div className="flex items-center gap-2">
                          <button
                            className="rounded-lg border border-border-subtle bg-surface px-3 py-1.5 text-sm hover:border-border"
