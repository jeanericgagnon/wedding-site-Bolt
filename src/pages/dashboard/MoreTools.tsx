  ...
  return (
    <DashboardLayout currentPage="tools">
      <div className="space-y-8">
        ...
        <section className="rounded-[20px] border border-border bg-surface p-5 shadow-none">
        ...
        {DASHBOARD_TOOL_GROUPS.map((group) => (
          <section key={group.title} className="space-y-4">
            ...
            <div className="overflow-hidden rounded-[20px] border border-border bg-surface shadow-none">
              {group.tools.map((tool, index) => {
                ...
                return (
                  <div key={tool.id} className={`grid gap-5 p-5 md:grid-cols-[minmax(0,1fr)_auto] md:items-center ${index > 0 ? 'border-t border-border' : ''}`}>
                    <div className="flex gap-4">
                      ...
                      <div>
                        <div className="flex flex-wrap items-center gap-2">
                          ...
                          <span className="rounded-full border border-border bg-surface-subtle px-2 py-0.5 text-[11px] font-medium text-text-tertiary">{tool.primaryHome}</span>
                          {tool.adminOnly && <span className="rounded-full border border-border bg-surface-subtle px-2 py-0.5 text-[11px] font-medium text-text-tertiary">Admin only</span>}
                        </div>
                    ...
                    <div className="flex flex-col gap-3">
                      <Link to={tool.path} className="inline-flex min-h-[44px] items-center justify-center rounded-lg border border-primary bg-primary px-4 text-sm font-semibold text-white no-underline hover:bg-primary-hover">
                        {tool.actionLabel}
                      </Link>
                        ...
                        <button
                          ...
                          className={`inline-flex min-h-[38px] items-center justify-center gap-2 rounded-lg border px-3 py-2 text-sm font-medium transition-colors ${
                            isShownInSidebar
                              ...
                              : 'border-border bg-surface text-text-tertiary hover:text-text-primary disabled:cursor-not-allowed disabled:opacity-40'
                          }`}
                        >
