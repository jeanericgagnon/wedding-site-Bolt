  ...
  return (
    <Card>
      ...
      <div className="mt-4 grid gap-3 md:grid-cols-5">
        <div className="rounded-lg border border-border-subtle p-3"><p className="text-xs text-text-tertiary">Status</p><p className="mt-2 text-sm font-semibold text-text-primary">{snapshot.readinessSummary.status}</p></div>
        <div className="rounded-lg border border-border-subtle p-3"><p className="text-xs text-text-tertiary">Needs first</p><p className="mt-2 text-sm font-semibold text-text-primary">{snapshot.readinessSummary.blockingFieldRisks}</p></div>
        <div className="rounded-lg border border-border-subtle p-3"><p className="text-xs text-text-tertiary">Worth checking</p><p className="mt-2 text-sm font-semibold text-text-primary">{snapshot.readinessSummary.attentionFieldRisks}</p></div>
        <div className="rounded-lg border border-border-subtle p-3"><p className="text-xs text-text-tertiary">Needs review</p><p className="mt-2 text-sm font-semibold text-text-primary">{snapshot.readinessSummary.lowConfidenceFields}</p></div>
        <div className="rounded-lg border border-border-subtle p-3"><p className="text-xs text-text-tertiary">Document checks</p><p className="mt-2 text-sm font-semibold text-text-primary">{snapshot.readinessSummary.documentRepairDebt}</p></div>
      </div>
        ...
        {snapshot.checklist.map((item) => (
          <div key={item.label} className="rounded-lg border border-border-subtle p-4">
            <div className="flex items-start justify-between gap-3">
              ...
              <span className={`rounded-md px-2 py-1 text-xs ${item.status === 'ready' ? 'bg-success/10 text-success' : item.status === 'attention' ? 'bg-warning/10 text-warning' : 'bg-danger/10 text-danger'}`}>{getNameChangeStatusChipLabel(item.status)}</span>
            </div>
          ...
          {snapshot.sequence.dependencies.map((dependency) => (
            <div key={dependency.key} className="rounded-lg border border-border-subtle p-4">
              <div className="flex items-start justify-between gap-3">
                ...
                <span className={`rounded-md px-2 py-1 text-xs ${dependency.status === 'satisfied' ? 'bg-success/10 text-success' : dependency.status === 'attention' ? 'bg-warning/10 text-warning' : 'bg-danger/10 text-danger'}`}>{getNameChangeStatusChipLabel(dependency.status)}</span>
              </div>
              ...
              {snapshot.fieldRisks.map((risk) => (
                <div key={`${risk.fieldKey}-${risk.severity}`} className="rounded-lg border border-border-subtle p-4">
                  <div className="flex items-start justify-between gap-3">
                    ...
                    <span className={`rounded-md px-2 py-1 text-xs ${risk.severity === 'blocking' ? 'bg-danger/10 text-danger' : 'bg-warning/10 text-warning'}`}>{getNameChangeStatusChipLabel(risk.severity)}</span>
                  </div>
