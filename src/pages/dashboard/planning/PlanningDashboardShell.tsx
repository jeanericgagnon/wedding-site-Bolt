    ...
    <DashboardLayout currentPage="planning">
      <div className="max-w-[1100px] mx-auto space-y-6">
        <DashboardPageHero
          ...
          actions={
            <>
              <Link to="/dashboard/itinerary" className="rounded-xl border border-border-subtle bg-white px-3 py-2 text-sm font-medium text-text-primary no-underline hover:bg-surface-subtle">Schedule</Link>
              <Link to="/dashboard/guests" className="rounded-xl border border-border-subtle bg-white px-3 py-2 text-sm font-medium text-text-primary no-underline hover:bg-surface-subtle">Guests</Link>
              <Link to="/dashboard/coordinator" className="rounded-xl bg-primary px-3 py-2 text-sm font-medium text-white no-underline hover:bg-primary/90">Day-of view</Link>
            </>
          }
        />
        ...
        <section className="rounded-2xl border border-border-subtle bg-white p-5 shadow-sm">
          <div className="flex flex-col gap-3 lg:flex-row lg:items-end lg:justify-between">
            <div>
              ...
              <h2 className="mt-3 text-lg font-semibold text-text-primary">Switch between the planning areas without losing the bigger picture.</h2>
            ...
            <div className="inline-flex flex-wrap gap-2 text-xs text-text-tertiary">
              <span className="rounded-xl border border-border-subtle bg-surface-subtle/30 px-3 py-1">Tasks and vendors</span>
              <span className="rounded-xl border border-border-subtle bg-surface-subtle/30 px-3 py-1">Budget and payments</span>
              <span className="rounded-xl border border-border-subtle bg-surface-subtle/30 px-3 py-1">Supporting details</span>
            </div>
          </div>
        </section>

        <div className="grid grid-cols-1 gap-3 rounded-2xl border border-border-subtle bg-white/80 p-4 shadow-sm md:grid-cols-2">
          <div>
            <label className="text-sm font-semibold text-text-primary">Section</label>
            <select
              value={activeTab}
              onChange={(e) => onTabChange(e.target.value as Tab)}
              className="mt-1 w-full rounded-xl border border-border bg-surface px-3 py-2.5 text-sm text-text-primary focus:outline-none focus:ring-2 focus:ring-primary"
            >
              {TABS.map((tab) => (
