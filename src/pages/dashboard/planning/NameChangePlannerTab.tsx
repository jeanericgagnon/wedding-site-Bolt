...
import { ExecutionSnapshotCard, ReminderPostureCard } from './NameChangePlannerCards';
...
import { buildNameChangeActionFeed, ensureTerminalPeriod, getAccountUpdateTemplateAudienceLine, getAccountUpdateTemplateContextLines, getAccountUpdateTemplateCopyButtonLabel, getAccountUpdateTemplateReadinessLine, getAccountUpdateTemplateStatusLine, getExecutionNextActionDetail } from '../../../lib/nameChange/actionFeed';
...
import {
  EXECUTION_SECTION_STEP_IDS, NAME_CHANGE_DOCUMENT_OPTIONS, NAME_CHANGE_EXTRACTION_FIELD_LABELS,
  NAME_CHANGE_EXTRACTION_FIELD_PLACEHOLDERS, TARGET_STATUS_VAULT_STATUS_PRIORITY, ensureDocument,
  findContractDocument, findContractExtractedField, formatAccountUpdateTemplateCopy,
  ...
  getActivitySourceLabel, getDocumentDetailLabel, getDocumentStorageModeLabel, getEffectiveBlockingProofHopLabel,
  getExecutionStatusLabel, getExecutionSummaryTone, getIntakeStatusLabel, getNameChangeStatusChipLabel,
  parseDocumentSnapshotDraft, getReminderCtaLabel, getRepairSeverityLabel, getWorkflowStatusLabel,
  matchesContractDocumentKind, readNameChangeAdminPreference, readNameChangeCollapsedSections, updateDocument,
...
function scrollToPlannerTarget(targetId: string) {
  if (typeof window !== 'undefined') window.history.replaceState(null, '', `${window.location.pathname}${window.location.search}#${targetId}`);
...
interface Props { draft: NameChangeCaseInput; documents: NameChangeDocumentInput[]; extractedFields: NameChangeExtractedFieldInput[]; plan: NameChangePlan; reminders: NameChangeReminderInput[]; saving: boolean; onDraftChange: (updates: Partial<NameChangeCaseInput>) => void; onStructuredIntakeChange: (key: string, value: unknown) => void; onDocumentsChange: (documents: NameChangeDocumentInput[]) => void; onExtractedFieldsChange: (fields: NameChangeExtractedFieldInput[]) => void; onRemindersChange: (reminders: NameChangeReminderInput[], context?: { action: 'single-update' | 'bulk-update' | 'schedule-stale' }) => void; onStepExecutionStatusChange: (stepId: string, executionStatus: 'todo' | 'in_progress' | 'complete') => void; onStepExecutionNoteChange: (stepId: string, note: string) => void; onSave: () => Promise<void>; initialTargetId?: string; }
