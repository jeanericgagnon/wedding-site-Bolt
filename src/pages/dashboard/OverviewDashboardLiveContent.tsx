...
import { Edit, ExternalLink, Eye, EyeOff, Palette, Radio, Users } from 'lucide-react';
...
function resumeOnboarding(questionKey: string, navigate: (href: string) => void) {
  if (typeof window !== 'undefined') {
    writeOnboardingResumeTarget(questionKey);
  }
  navigate('/onboarding');
}
...
export function OverviewDashboardLiveContent({
  ...
  nextStepActionLabel,
  nextStepLabel,
  onDismissInvisibleSuggestion,
  onHideSuggestion,
  onRefreshDraftFromBrief,
  recentSiteActivity,
  refreshingBrief,
  setShowMoreDetail,
  setupChecklistLength,
  setupCompletedCount,
          ...
          <aside className="border-t border-border-subtle bg-surface-subtle/35 p-4 md:p-5 lg:border-l lg:border-t-0">
            <div className="flex h-full flex-col justify-between gap-5">
              ...
              <div className="space-y-3">
                ...
                {stats?.siteSlug && (
                  <Button variant="outline" size="md" fullWidth onClick={() => openSitePreview(stats.siteSlug!)}>
      ...
      <div className="card-clean px-5 py-4 md:px-6 md:py-5">
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          ...
          <div className="text-right">
            <p className="text-2xl font-bold text-text-primary">{setupCompletedCount}/{setupChecklistLength}</p>
                  ...
                  >
                    <div className="flex items-start justify-between gap-3">
                      ...
                      <span className={`shrink-0 rounded-lg border px-2 py-0.5 text-[11px] font-medium ${
                        ...
                        {item.priority === 'now' ? 'Now' : item.priority === 'soon' ? 'Soon' : item.priority === 'watch' ? 'Watch' : 'Quiet'}
                      </span>
                    </div>
                ...
                {invisibleSuggestions.slice(0, 4).map((suggestion) => (
                  <div key={suggestion.id} className="rounded-lg border border-border-subtle bg-white px-4 py-4">
                    <div className="flex items-start justify-between gap-3">
                      ...
                      <button
                        type="button"
                        onClick={() => onDismissInvisibleSuggestion(suggestion.id)}
            ...
            <CardContent className="space-y-4">
                ...
                {draftBrief.map((item) => (
                  <div key={item.id} className="rounded-lg border border-border-subtle bg-white px-4 py-3">
                    <div className="flex items-start justify-between gap-3">
                      ...
                      <button
                        type="button"
                        onClick={() => resumeOnboarding(item.questionKey, navigate)}
              ...
              <div className="flex flex-wrap gap-2">
                <Button
                  variant="accent"
                  size="sm"
                  onClick={() => resumeOnboarding('first-incomplete', navigate)}
                >
                  Resume concierge
                </Button>
                <Button variant="outline" size="sm" onClick={() => onRefreshDraftFromBrief()} disabled={refreshingBrief}>
                ...
                {archiveMode.isArchiveLike && (
                  <div className="rounded-lg border border-border-subtle bg-surface-subtle/40 px-4 py-4 space-y-3">
                    ...
                    <div className="mt-3 flex flex-wrap gap-2">
                      ...
                      <Button variant="outline" size="sm" onClick={() => stats?.siteSlug && openSitePreview(stats.siteSlug)}>Revisit public site</Button>
                    </div>
                  </div>
                )}
                ...
                {archiveMode.isArchiveLike && (
                  <div className="rounded-lg border border-border-subtle bg-white px-4 py-4 space-y-4">
                    ...
                    <div className="flex flex-wrap gap-2">
                      ...
                      <Button variant="outline" size="sm" onClick={() => stats?.siteSlug && openSitePreview(stats.siteSlug)}>Open keepsake site</Button>
                    </div>
                  </div>
                )}
          ...
          <Card variant="bordered" padding="lg">
            <CardHeader>
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <CardDescription>{publishState.explainer}</CardDescription>
                </div>
            ...
            <CardContent className="space-y-4">
              ...
              <div className="flex flex-col sm:flex-row gap-3 pt-4">
                {stats?.siteSlug && (
                  <Button variant="accent" size="md" fullWidth onClick={() => openSitePreview(stats.siteSlug!)}>
                    ...
                    {interactiveSuggestions.map((item) => (
                      <div key={item.id} className="rounded-lg border border-border-subtle bg-surface-secondary/30 px-3 py-2.5">
                        <div className="flex items-start justify-between gap-2">
                          ...
                          <button
                            type="button"
                            onClick={() => onHideSuggestion(item.id)}
