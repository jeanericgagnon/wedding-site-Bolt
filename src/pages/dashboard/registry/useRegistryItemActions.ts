...
export function useRegistryItemActions(args: UseRegistryItemActionsArgs) {
  ...
  async function handleSave(draft: RegistryItemDraft) {
    ...
    const fields: Partial<RegistryItem> = {
      ...
      source_type: isCashFund ? 'cash_fund' : (draft.source_type ?? (draft.item_url ? 'link' : draft.barcode ? 'barcode' : 'manual')),
      item_name: draft.item_name.trim(),
      barcode: isCashFund ? null : (draft.barcode?.trim() || null),
      price_label: null,
      ...
      item_url: isCashFund ? null : (draft.selected_product_url || draft.item_url || null),
      canonical_url: isCashFund ? null : (draft.canonical_url || draft.selected_product_url || draft.item_url || null),
      image_url: normalizedImageUrl,
      selected_retailer: isCashFund ? null : (draft.selected_retailer || draft.merchant || null),
      selected_product_url: isCashFund ? null : (draft.selected_product_url || draft.item_url || null),
      estimated_price_cents: isCashFund ? null : (draft.estimated_price_cents ? parseInt(draft.estimated_price_cents, 10) || null : (parsedPrice !== null && !isNaN(parsedPrice) ? Math.round(parsedPrice * 100) : null)),
      product_metadata: isCashFund ? null : (draft.product_metadata ?? null),
    ...
    if (isDemoMode) {
      if (editItem) {
        ...
        const created: RegistryItem = {
          ...
          selected_retailer: fields.selected_retailer ?? null,
          selected_product_url: fields.selected_product_url ?? null,
          estimated_price_cents: fields.estimated_price_cents ?? null,
          product_metadata: fields.product_metadata ?? null,
