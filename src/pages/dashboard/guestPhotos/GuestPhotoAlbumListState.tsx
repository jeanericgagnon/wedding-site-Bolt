...
export function GuestPhotoAlbumListState({
  ...
  if (bucketCount === 0) {
    return (
      <div className="rounded-[20px] border border-dashed border-border-subtle bg-surface-subtle p-8">
        ...
        <div className="mt-5 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
          {['Welcome party', 'Ceremony', 'Dance floor', 'Brunch'].map((suggestion) => (
            <button
              ...
              className="rounded-[20px] border border-border-subtle bg-white px-4 py-4 text-left transition-colors hover:bg-surface-subtle"
            >
  ...
  if (filteredBucketCount === 0) {
    return (
      <div className="rounded-[20px] border border-border-subtle bg-surface-subtle px-4 py-5 text-sm text-text-secondary">
