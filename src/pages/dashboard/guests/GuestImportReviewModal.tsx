      ...
      <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
        <div className="flex max-h-[80vh] w-full max-w-2xl flex-col rounded-lg border border-border-subtle bg-surface">
          <div className="flex items-center justify-between border-b border-border-subtle p-6">
            ...
            {!csvImporting && (
              <button onClick={onResetWarnings} className="rounded-lg p-2 transition-colors hover:bg-surface-subtle">
          ...
          <div className="flex-1 overflow-y-auto p-6">
            {(csvMappingSummary.core.length > 0 || csvMappingSummary.rsvp.length > 0 || csvMappingSummary.household.length > 0 || csvMappingSummary.eventCols.length > 0 || csvMappingSummary.weak.length > 0) && (
              <div className="mb-4 space-y-2 rounded-lg border border-border bg-surface-subtle p-3">
            ...
            <div className="divide-y divide-border-subtle">
              {csvPreview.slice(0, 50).map((guest, index) => (
                <div key={index} className="flex items-center gap-4 py-2.5">
                  <div className="flex h-7 w-7 flex-shrink-0 items-center justify-center rounded-lg bg-surface-subtle text-xs font-medium text-text-secondary">
                    {index + 1}
                  </div>
                  <div className="min-w-0 flex-1">
                    ...
                    <div className="mt-1 flex flex-wrap gap-1">
                      {Boolean(guest.rsvp_status) && <span className="rounded-lg border border-border px-1.5 py-0.5 text-[10px] text-text-tertiary">{String(guest.rsvp_status)}</span>}
                      {Boolean(guest.__meal_choice) && <span className="rounded-lg border border-border px-1.5 py-0.5 text-[10px] text-text-tertiary">Meal: {String(guest.__meal_choice)}</span>}
                      {Boolean(guest.__plus_one_name) && <span className="rounded-lg border border-border px-1.5 py-0.5 text-[10px] text-text-tertiary">+1: {String(guest.__plus_one_name)}</span>}
                      {Number(guest.__children_count ?? 0) > 0 && <span className="rounded-lg border border-border px-1.5 py-0.5 text-[10px] text-text-tertiary">Children: {String(guest.__children_count)}</span>}
                      {Array.isArray(guest.__invited_event_ids) && (guest.__invited_event_ids as unknown[]).length > 0 && <span className="rounded-lg border border-primary/30 px-1.5 py-0.5 text-[10px] text-primary">{(guest.__invited_event_ids as unknown[]).length} event invites</span>}
                    </div>
                  </div>
                  {Boolean(guest.plus_one_allowed) && (
                    <span className="flex-shrink-0 rounded-lg bg-surface-subtle px-2 py-0.5 text-xs text-text-secondary">+1</span>
                  )}
                </div>
              ))}
