...
function parseLegacyEventAttendanceToken(value: string | undefined): boolean | undefined {
  ...
  if (normalized === 'no' || normalized === 'n' || normalized === 'false' || normalized === '0' || normalized === 'off' || normalized === 'disabled' || normalized === 'excluded' || normalized === 'out' || normalized === 'declined' || normalized === 'skipping' || normalized === 'absent' || normalized === 'unconfirmed' || normalized === 'cancelled' || normalized === 'canceled' || normalized === 'not attending' || normalized === 'not going' || normalized === 'not included' || normalized === 'not joining' || normalized === 'not coming' || normalized === 'not participating' || normalized === 'not present') {
    return false;
  }
  return undefined;
}
...
function deriveSelectedHouseholdGuestIds(existingRsvp: ExistingRSVP | null, household: HouseholdGuest[]): string[] {
  ...
  const selectedFromRsvp = (existingRsvp?.guest_ids ?? []).filter((guestId) => invitedIds.has(guestId));
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (DEMO_MODE) {
        ...
        setApplyToHousehold(applyToHousehold);
        setSelectedHouseholdGuestIds(applyToHousehold ? [...selectedHouseholdGuestIds] : []);
        setStep('success');
        return;
      }
