...
describe('Product starter draft truth', () => {
  ...
  it('keeps product-level messaging summaries aligned with review-before-send and sender-ready texting truth', () => {
    ...
    expect(screen.queryByText('Day-of ready')).not.toBeInTheDocument();
    expect(screen.queryByText('Gift links live')).not.toBeInTheDocument();
  });
  ...
  it('personalizes product draft-start CTAs for signed-in users', () => {
    ...
    expect(screen.getAllByRole('button', { name: 'Open website editor guide' }).length).toBeGreaterThan(0);
    ...
    expect(screen.getAllByRole('button', { name: 'Open website editor guide' })[0]).toBeInTheDocument();
