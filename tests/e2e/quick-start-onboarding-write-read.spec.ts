...
test('quick-start AI onboarding creates an editable starter site and guest import handoff', async ({ page }) => {
  ...
  try {
    ...
    await page.waitForURL(handoffUrlPattern, { timeout: 10_000 }).catch(async () => {
      ...
      const guestsHeading = page.getByRole('heading', { name: 'People, replies, and details.' });
    ...
    await expect(page.getByRole('heading', { name: 'People, replies, and details.' })).toBeVisible({ timeout: 90_000 });
