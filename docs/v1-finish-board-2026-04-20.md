...
1. **Canonical v1 smoke pass**
   ...
   - operator no-go source: keep this slice in no-go mode while `publicV1ClaimStatus` remains `not_clear_to_claim_yet`
   ...
   - operator mismatch rule: if this board and the proof log disagree, trust the proof log first and update this board second
   ...
   - operator downgrade rule: only remove the red posture here after both the env blocker and the manual-proof blocker count move in the proof log
