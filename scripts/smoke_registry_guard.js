...
const checks = [
  ...
  { name: 'builder registry variant compatibility tolerates trim and casing drift', ok: sectionVariantCompatibility.includes('const supportedByLowercase = new Map') && sectionVariantCompatibility.includes('const canonicalVariant = supportedByLowercase.get(normalizedVariant.toLowerCase()) ?? normalizedVariant;') && sectionVariantCompatibility.includes("Object.entries(aliases).find(([aliasVariant]) => aliasVariant.toLowerCase() === normalizedVariant.toLowerCase())?.[1]") },
