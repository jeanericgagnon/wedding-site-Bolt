...
const checks = [
  ...
  { name: 'builder registry variant compatibility tolerates trim casing and malformed fallback drift', ok: sectionVariantCompatibility.includes('export function resolveBuilderVariant(type: SectionType, variant: unknown): string {') && sectionVariantCompatibility.includes('const supportedByLowercase = new Map') && sectionVariantCompatibility.includes("const normalizedVariant = typeof variant === 'string' ? variant.trim() : '';") && sectionVariantCompatibility.includes('const canonicalVariant = supportedByLowercase.get(normalizedVariantKey) ?? normalizedVariant;') && sectionVariantCompatibility.includes("Object.entries(aliases).find(([aliasVariant]) => aliasVariant.toLowerCase() === normalizedVariantKey)?.[1]") && sectionVariantCompatibility.includes("if (!alias) return supported[0] ?? 'default';") },
