...
const checks = [
  ...
  { name: 'site view registry fallback normalization stays aligned with public cards first', ok: siteView.includes("const supported = getSectionVariants(section.type);") && siteView.includes("const nextVariant = fallbackMap[section.type]?.[section.variant] ?? supported[0] ?? 'default';") },
