...
const checks = [
  ...
  { name: 'section registry keeps strict alias fallbacks for template-backed registry variants', ok: sectionRegistry.includes('const def = strictVariant') && sectionRegistry.includes('VARIANT_FALLBACKS[type]?.[normalizedVariant] ? getDefinition(normalizedType, VARIANT_FALLBACKS[type][normalizedVariant]) : null') },
  { name: 'section registry normalizes persisted registry variant drift before public render fallback', ok: sectionRegistry.includes('function normalizeRegistryVariantKey(variant: string): string {') && sectionRegistry.includes('const normalizedVariant = resolveRegistryVariant(type, variant);') && sectionRegistry.includes('normalizeRegistryVariantKey(aliasVariant) === normalizedVariantKey') },
  ...
  { name: 'builder registry variant compatibility tolerates trim casing punctuation and malformed fallback drift', ok: sectionVariantCompatibility.includes('export function resolveBuilderVariant(type: SectionType, variant: unknown): string {') && sectionVariantCompatibility.includes("const normalizeVariantKey = (value: string) => value.toLowerCase().replace(/[^a-z0-9]/g, '');") && sectionVariantCompatibility.includes("const normalizedVariant = typeof variant === 'string' ? variant.trim() : '';") && sectionVariantCompatibility.includes('const supportedByKey = new Map') && sectionVariantCompatibility.includes('const canonicalVariant = supportedByKey.get(normalizedVariantKey) ?? normalizedVariant;') && sectionVariantCompatibility.includes("Object.entries(aliases).find(([aliasVariant]) => normalizeVariantKey(aliasVariant) === normalizedVariantKey)?.[1]") && sectionVariantCompatibility.includes("if (!alias) return supported[0] ?? 'default';") },
  ...
  { name: 'legacy public registry surfaces keep empty live loads from falling back to stale links', ok: registrySectionComponent.includes('export function shouldUseLiveRegistryItems(items: RegistryItem[] | null): items is RegistryItem[] {') && registrySectionComponent.includes('if (shouldUseLiveRegistryItems(items)) {') },
  { name: 'registry proof output encodes remaining runtime truth gaps', ok: registryProof.includes("status: 'manual-proof-pending'") && registryProof.includes("highestRiskTrustGap: 'runtime_registry_truth_after_real_edits'") && registryProof.includes("secondaryTrustGap: 'registry_repair_and_import_persistence_manual_verification_missing'") && registryProof.includes('manualProofRequired: true') && registryProof.includes("truthGateSummary: 'automation_green_manual_truth_red'") && registryProof.includes("'owner_manage_import_persistence_runtime_pass'") && registryProof.includes("'owner_repair_cleanup_runtime_pass'") && registryProof.includes("'guest_visible_purchase_truth_runtime_pass'") },
  { name: 'builder registry previews map aliases onto guest-visible layouts', ok: variantPreviewSource.includes("case 'classic':") && variantPreviewSource.includes("case 'default':") && variantPreviewSource.includes("return 'cards';") && variantPreviewSource.includes("case 'luxury':") && variantPreviewSource.includes("return 'featured';") && builderInspectorPanel.includes('getVariantPreviewSource(selectedSection.type, v.id)') && builderSectionRail.includes('getVariantPreviewSource(addTypeManifest.type, v.id)') && builderSidebarLibrary.includes('PREVIEW_FIXTURES_BY_VARIANT[`${sectionType}:${previewVariantId}`]') },
  ...
  { name: 'public registry cards do not fall back to template links after empty live loads', ok: registryCardsSection.includes('return liveItems !== null;') && registryCardsSection.includes('const shouldUseLiveStoreGroups = shouldUseLiveRegistryStoreGroups(liveItems);') },
  { name: 'public registry featured links derive live store urls from canonical items', ok: registryFeaturedSection.includes("return item.item_url ?? item.canonical_url ?? '';") && registryFeaturedSection.includes('const displayStoreLinks = shouldUseLiveData ? groupRegistryStoreLinks(liveItems) : safeStoreLinks;') },
  { name: 'public registry featured view does not fall back after empty live loads', ok: registryFeaturedSection.includes('return liveItems !== null;') && registryFeaturedSection.includes('const shouldUseLiveData = shouldUseLiveRegistryFeaturedData(liveItems);') },
