...
| Field | Current State |
| --- | --- |
| Current date/time | `2026-05-11 09:13 PM PDT` |
| Branch | `codex/v1-finish-hard-gates-3` |
| Latest Git SHA | `f83b7d92` |
| Latest commit message | `Apply RPC sweep remotely and refresh live RLS proof` |
...
- `P2 Live RLS proof should be expanded further`
  ...
  - note:
    ...
    - it now also proves direct guest, planning, seating, message, registry, and photo RPC lanes remain permission-scoped while direct timeline/settings and other ungranted writes stay denied without the matching permission set
...
| `npm run proof:v1:client-rls-matrix` | `LIVE PASS` | `production + browser runtime` | `2026-05-11` | Aggregates live anon guest-contact scope, public RSVP scope, owner/collaborator viewer-deny plus planner/coordinator-allow runtime proof, direct guest/planning/seating write allow/deny coverage, planner message RPC allow + registry RPC deny, coordinator media RPC allow + dashboard message RPC deny, and the guest-dashboard settings RPC lane with `LIVE_GUEST_DASHBOARD_SETTINGS_RPCS=1` |
...
- Expanded the live collaborator/client-RLS matrix so it now proves guest, planning, seating, message, registry, and photo permission lanes stay scoped while direct timeline/settings and other ungranted writes remain denied.
