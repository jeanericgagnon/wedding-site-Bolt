...
| Current proof state | Launch-critical runtime proof remains green on the live runtime: `npm test`, `typecheck`, `lint`, `build`, `test:security`, `public-access-coverage`, `service-role-authorization`, `email-messaging-authorization`, `launch-closeout`, `canonical-smoke`, `public-quality`, `guests-rsvp-ops`, `guest-lookup-scope`, `collaborator-runtime`, `client-rls-matrix`, `registry-preview-ssrf`, `coordinator-dayof`, `name-change-runtime`, and `registry`. A fresh 2026-05-13 in-depth production rerun on deploy `dpl_4TCWmUaSfuV3MJysqcYJUCyKTWJs` is green via `PLAYWRIGHT_BASE_URL=https://dayof.love npm run proof:v1:canonical-smoke`, `V1_COORDINATOR_DAYOF_LIVE=1 npm run proof:v1:coordinator-dayof -- --require-live`, `V1_NAME_CHANGE_RUNTIME_LIVE=1 npm run proof:v1:name-change-runtime -- --require-live`, `LIVE_REGISTRY_WRITE_READ=1 npm run proof:v1:registry -- --require-live`, `npx playwright test --workers=1 tests/e2e/full-suite-three-lanes-responsive.spec.ts`, and `LIVE_COLLABORATOR_PERMISSION_RLS=1 npx playwright test --workers=1 tests/e2e/collaborator-permission-rls.spec.ts`. The combined local cross-feature exit gate also remains green via `npm run proof:v1:full-suite-exit-gate`, and that exit gate is now documented honestly as an aggregate of the dedicated lane proofs plus the responsive/device surface proof. |
| Current deployment state | The latest deployed frontend runtime is [dayof.love](https://dayof.love) via verified Vercel production deploy `dpl_4TCWmUaSfuV3MJysqcYJUCyKTWJs` from commit `4ebc544f`. The coordinator migrations `20260513170000_coordinator_event_checkin_write.sql` and `20260513213000_coordinator_handoff_issue_depth.sql`, the name-change reminder compatibility migration `20260513193000_fix_name_change_reminders_replace_runtime.sql`, and the registry duplicate-merge migration `20260513195500_add_registry_duplicate_merge.sql` remain applied remotely. This deploy ships the coordinator incident lifecycle, runner board, guest continuity, shift snapshot surfaces, the full-suite name-change planner depth, and the full-suite registry provider/repair/fallback batch live on production. `registry-barcode-lookup --no-verify-jwt` remains live with the broader Open Facts ladder and repair-queue-compatible lookup path. Fresh post-deploy deep proof on this runtime is green through canonical smoke, dedicated coordinator/name-change/registry live proofs, the responsive three-lane proof, and collaborator permission RLS proof. `submit-rsvp` remains live with the serialized capacity path, and the public-session-secret, admin route gate, guest-contact, route-module decomposition, vault contribution, and `.dayof.love` host-routing lanes remain live-proven. External custom domains remain unsupported product scope, not an active proof lane. |
...
- `Day-of / coordinator`
  ...
  - coordinator full-suite batch proof is green locally and live: `npm run proof:v1:coordinator-dayof`, `npm run typecheck -- --pretty false`, focused coordinator Vitest lane, `npm run lint -- --quiet`, `git diff --check`, `npm run build`, Vercel production deploy `dpl_4TCWmUaSfuV3MJysqcYJUCyKTWJs`
  ...
  - deployed now: migrations `20260513170000_coordinator_event_checkin_write.sql` and `20260513213000_coordinator_handoff_issue_depth.sql` plus Vercel production deploy `dpl_4TCWmUaSfuV3MJysqcYJUCyKTWJs`
  - live proof is green on the current production runtime for handoff save, incident lifecycle, runner workflow completion, guest continuity, and shift snapshot copy/print export
...
- `Name change`
  ...
  - deployed now: Vercel production deploy `dpl_4TCWmUaSfuV3MJysqcYJUCyKTWJs` plus migration `20260513193000_fix_name_change_reminders_replace_runtime.sql`
...
- `Universal Registry Barcode Scanner`
  - `FULL-SUITE LANE CLOSED`
  - current truth: the competitor-informed MVP bar, the reopened deeper product-depth bar, and the fuller-suite barcode/reconciliation/repair batch are all now shipped and live-proven on the current production runtime
  ...
  - deployed now: migrations `20260513064500_add_registry_barcode_scanner_support.sql` and `20260513195500_add_registry_duplicate_merge.sql`, frontend runtime `dpl_4TCWmUaSfuV3MJysqcYJUCyKTWJs`, and live `registry-barcode-lookup --no-verify-jwt`
  - live proof is green on the current production runtime, including broader provider-path barcode lookup, duplicate merge collapse/readback, cleanup-queue truth, retailer reconciliation, compatibility camera/photo fallback, owner save/read flow, and public registry endpoint readability
...
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| Vercel frontend / `dayof.love` | `current production deploy` | `yes` | `Vercel production dpl_4TCWmUaSfuV3MJysqcYJUCyKTWJs` | `--prod` | Canonical smoke, public-quality, coordinator full-suite live proof, name-change full-suite live proof, and registry full-suite live proof all reran green on the current deploy | green on the current production deploy | None on the current full-suite truth lane | `LIVE PASS` |
