...
- `P2 Live RLS proof should be expanded further`
  ...
  - note:
    ...
    - it now also proves direct guest, planning, registry, seating, coordinator, message, and photo RPC lanes remain permission-scoped while direct timeline/settings and other ungranted writes stay denied without the matching permission set
...
| `npm run proof:v1:client-rls-matrix` | `LIVE PASS` | `production + browser runtime` | `2026-05-11` | Aggregates live anon guest-contact scope, public RSVP scope, owner/collaborator viewer-deny plus planner/coordinator/registry-allow runtime proof, direct guest/planning/seating write allow/deny coverage, planner message RPC allow + registry RPC deny, registry RPC allow + message RPC deny, coordinator Q&A/check-in/media RPC allow + dashboard message RPC deny, and the guest-dashboard settings RPC lane with `LIVE_GUEST_DASHBOARD_SETTINGS_RPCS=1` |
