...
| Field | Current State |
| --- | --- |
| Current date/time | `2026-05-12 11:18 AM PDT` |
| Branch | `codex/v1-finish-hard-gates-3` |
| Latest Git SHA | `19cecf20` |
| Latest commit message | `Sync launch docs after tooling and session hardening` |
| Vercel deployment ID | `dpl_386dKTNkTVK95UfwJj9qEtnH1b8q` |
| Supabase project ID | `atuzuobpprjstfmdnwso` |
| Supabase functions deployed | Live blocker-fix lane now includes `submit-rsvp --no-verify-jwt` plus applied migration `20260511170500_serialize_submit_rsvp_capacity.sql`. Earlier same-day confirmed/live-proven: `public-site-access --no-verify-jwt`; `photo-upload --no-verify-jwt`; `process-email-queue`; `guest-contact-lookup --no-verify-jwt`; `guest-contact-submit --no-verify-jwt`; `translate-site-content`. New same-day live session-secret separation redeploys are green for `public-site-access --no-verify-jwt`, `guest-contact-lookup --no-verify-jwt`, `guest-contact-submit --no-verify-jwt`, `validate-rsvp-token --no-verify-jwt`, `photo-upload --no-verify-jwt`, and `interactive-section-public --no-verify-jwt`. Public vault contribution remains deferred/fail-closed because live inventory still does not confirm `vault-contribution-public`. |
| Current readiness score | `9.8 / 10` |
| Current launch verdict | `GO` |
| Production-ready | `YES` |
| Reason production-ready is not yet claimed | No active P0/P1 blockers remain. Production-ready is still claimed for the current launch baseline. The remaining gap to a cleaner `10 / 10` is that the new `admin_access_check()` server-side admin gate is still local-only because `supabase db push` is blocked in this shell until `SUPABASE_ACCESS_TOKEN` is available. |
| Current blockers | none |
| Current proof state | Launch-critical runtime proof remains green: `npm test`, `typecheck`, `lint`, `build`, `test:security`, `public-access-coverage`, `service-role-authorization`, `email-messaging-authorization`, `launch-closeout`, `canonical-smoke`, `public-quality`, `guests-rsvp-ops`, `guest-lookup-scope`, `collaborator-runtime`, and `client-rls-matrix` are green on the current live baseline. The post-launch hardening lane is broader again: `npm run proof:v1:client-write-inventory`, `npm run proof:v1:test-lanes`, and `npm run proof:v1:strict-pocket` are all `PASS`, `test:launch` now requires `npm run proof:v1:registry-preview-ssrf -- --require-live`, `.github/workflows/ci-hardpass.yml` now hard-fails if `VITE_SUPABASE_URL` / `VITE_SUPABASE_ANON_KEY` are missing instead of soft-skipping strict RSVP smoke, and `deploy_prod_guarded.mjs` no longer permits `SKIP_POSTDEPLOY_PROOF` to bypass postdeploy proof. Dedicated public session key separation is now live on the highest-risk public/session flows: `public-site-access`, `guest-contact-lookup`, `guest-contact-submit`, `validate-rsvp-token`, `photo-upload`, and `interactive-section-public` no longer use `SUPABASE_SERVICE_ROLE_KEY` as a session-signing secret and now require `PUBLIC_SITE_SESSION_SECRET_V1` or `PUBLIC_SITE_SESSION_SECRET` instead. Direct non-admin reads of `admin_users` are live-proven to fail or return no rows in the collaborator matrix. The remaining admin-route hardening step is the new `admin_access_check()` RPC rollout, which is coded and locally proven but still blocked from remote apply here by a missing `SUPABASE_ACCESS_TOKEN`. Signed public session tokens now use a versioned envelope and the verifier fails closed on malformed base64/signature/version parsing before payload decode. The live reruns with `LIVE_GUEST_DASHBOARD_SETTINGS_RPCS=1` now also prove planner `itinerary_event_write` allow + registry deny, settings `section_write` allow + registry deny, photos `wedding_site_vault_provider_patch` allow + dashboard message deny, registry `registry_refresh_policy_write` allow + dashboard message/section deny, and regular collaborators cannot query `admin_users` directly, in addition to the earlier guest, planning task/message, settings patch, registry item, photos/vault-config, and coordinator seating/Q&A/check-in/media lanes. The stale service-role disposition doc is now reconciled to that live truth, and the serialized RSVP capacity function is explicitly visible in `supabase/migrations/20260511170500_serialize_submit_rsvp_capacity.sql`. This wave found and fixed four real production RPC/runtime defects before the proof went green: missing `itinerary_events.dress_code`, missing `itinerary_events.notes`, create-path bugs in `itinerary_event_write` (`NULL id` plus text/time coercion drift) and `section_write` (explicit-id create path misread as missing-row update), and a `registry_refresh_policy_write` PostgreSQL `CASE` type mismatch on `registry_refresh_policy_updated_by`. Remote repair landed through migrations `20260512040000_reconcile_itinerary_dress_code_column.sql`, `20260512040500_reconcile_itinerary_runtime_columns.sql`, `20260512041000_fix_itinerary_event_write_time_types.sql`, `20260512041500_fix_itinerary_event_write_ids.sql`, `20260512042000_fix_section_write_create_with_explicit_id.sql`, and `20260512043000_fix_registry_refresh_policy_write_updated_by_type.sql`. Focused proof is green for `publicSessionSecretBoundary.test.ts`, `adminAccessRpcBoundary.test.ts`, `launchEdgeFunctions.test.ts`, `publicAccessCoverageProofScript.test.ts`, `signedSessionShared.test.ts`, `clientWriteInventoryProofScript.test.ts`, `collaboratorPermissionRlsProof.test.ts`, `clientRlsMatrixProofScript.test.ts`, `itineraryRpcSafety.test.ts`, `sectionRpcSafety.test.ts`, `typecheck`, and `git diff --check`. GitHub Actions `Release Launch Gate` remains green on run `25705683563`. |
| Current deployment state | Frontend is live at [dayof.love](https://dayof.love) on exact runtime Git SHA `f0cbf841` via verified Vercel production deploy `dpl_386dKTNkTVK95UfwJj9qEtnH1b8q`. `submit-rsvp` is live with the serialized capacity path and the database now includes the full RPC migration sweep through `20260512031500_seating_assignment_version_rpcs.sql` plus forward repair migrations `20260511212626_fix_wedding_site_settings_patch_types.sql`, `20260512040000_reconcile_itinerary_dress_code_column.sql`, `20260512040500_reconcile_itinerary_runtime_columns.sql`, `20260512041000_fix_itinerary_event_write_time_types.sql`, `20260512041500_fix_itinerary_event_write_ids.sql`, `20260512042000_fix_section_write_create_with_explicit_id.sql`, and `20260512043000_fix_registry_refresh_policy_write_updated_by_type.sql`. The live collaborator/client-RLS matrix now includes guest, planning, itinerary, settings, sections, seating, messages, registry item/policy, photos/vault, coordinator permission lanes, and direct `admin_users` read denial. The dedicated public-session-secret function redeploys are live on the public site/session lane. The frontend switch to the RPC-backed admin gate is not deployed because the database-side `admin_access_check()` migration has not been applied remotely from this shell yet. Public vault contribution remains outside the launch baseline because the live route still fails closed and the function inventory does not confirm `vault-contribution-public`. |
| Current next actions | apply `20260512050000_harden_admin_access_check.sql` remotely once `SUPABASE_ACCESS_TOKEN` is available, redeploy the frontend/admin route gate to use `admin_access_check()`, keep the live client-RLS matrix current when future non-guest write surfaces are introduced, keep the no-direct-client-write inventory current after future runtime write-surface changes, tighten TS/ESLint rigor incrementally in high-risk modules, and resolve deferred lanes only when they are intentionally brought into the launch baseline |
...
- `P2 Admin route authorization should finish the server-side move`
  - files:
    - [src/lib/adminUsers.ts](/Users/ericgagnon/Documents/DayOfLove/wedding-site-Bolt/src/lib/adminUsers.ts:1)
    - [supabase/migrations/20260512050000_harden_admin_access_check.sql](/Users/ericgagnon/Documents/DayOfLove/wedding-site-Bolt/supabase/migrations/20260512050000_harden_admin_access_check.sql:1)
  - note:
    - direct non-admin `admin_users` reads are already live-proven to fail or return no rows
    - the new `admin_access_check()` RPC is coded and locally proven, but it is not remote yet because this shell does not have `SUPABASE_ACCESS_TOKEN` for `supabase db push`
    - until that migration is applied and the frontend is redeployed, the internal tooling route gate still depends on the existing client-side admin check path for its final allow/deny decision
  - direction:
    - apply the migration remotely
    - redeploy the frontend to switch the route gate to `admin_access_check()`
