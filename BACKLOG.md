...
7. `PARTIAL` - Centralize access control through `public-site-access`.
   ...
   Current evidence: major public route/subresource paths were moved toward the resolver contract. `npm run proof:v1:public-access-coverage` now audits every Edge Function that selects `privacy_mode` and `guest_access_token`, with the resolver itself as the only allowed exception, and requires `canReadPublicSubresource` coverage plus no stale published-only shortcuts. The proof caught and this batch removed a stale `photo-upload` post-gate `is_published` shortcut so invite/password slug uploads now stay governed by the shared access gate. The static proof is now wired into `test:launch`, `proof:v1:test-lanes`, and CI hardpass so future public subresource drift fails the release lane. Broader live authenticated public/guest write-read proof remains tracked under sitewide proof.
...
8. `PARTIAL` - Audit service-role usage.
   ...
   Current evidence: service-role inventory/disposition doc and static guard exist. Messaging and photo/media mutation functions now use shared role-aware collaborator checks that block `viewer` mutations even with stale explicit permission rows. Live email/messaging and photo/media unauthenticated denial proofs now verify deployed send/queue/photo service-role functions reject missing authorization before privileged work. Local `onboarding-ai-orchestrate` usage attribution now verifies the authenticated user owns or collaborates on a requested site before storing service-role usage rows against that site. The service-role disposition now separates owner/collaborator functions, public submission functions, optional-auth rate-limited helpers, and internal/provider functions, with a test requiring every service-role Edge Function to appear in exactly one category. Local Google Drive OAuth callback state is now HMAC-signed by `google-drive-auth-start` and verified by `google-drive-auth-callback` before service-role Drive token writes. Authenticated role-by-role mutation proof and secure service-role/RLS proof remain open.
...
10. `PARTIAL` - Email safety.
    ...
    Current evidence: `send-wedding-email`, `process-email-queue`, and `send-bulk-message` now import shared Edge Function email safety helpers for HTML escaping, safe URLs, href escaping, and subject sanitization. `src/lib/emailSafety.test.ts` directly imports those shared Edge Function helpers and proves hostile HTML escaping, safe URL/href fallback and escaping, control-character subject sanitization, fallback subjects, and the subject length cap; `npm run test:security` runs that proof. Direct wedding emails, bulk/scheduled messages, and queued guest follow-ups now reject `viewer` collaborators even when a malformed explicit `messages`/`guests` permission exists. `npm run proof:v1:email-messaging-authorization` now verifies deployed `process-email-queue`, `queue-guest-followups`, `send-bulk-message`, and `send-wedding-email` deny unauthenticated live requests with `401`/`403` before send or queue mutation paths. Focused static proof, planner-access tests, typecheck, quiet lint, build, message smoke, and unauthenticated live denial proof pass; owner/planner/coordinator/viewer live mutation proof remains required with disposable proof accounts.
...
11. `PARTIAL` - Validation must pass and be recorded.
   ...
   Current evidence: `docs/PRODUCTION_HARDENING_REPORT.md` exists and records the latest validation lane. CI hardpass now runs typecheck, quiet lint, file-size guard, asset guard, tests, build, registry smoke, CSV mapper smoke, check-in smoke, messages smoke, and strict RSVP when secrets are present. The approved 2026-05-06 1:18 PM PT production deploy has green `PLAYWRIGHT_BASE_URL=https://dayof.love npm run proof:v1:postdeploy` proof at 8/8, including guests/RSVP ops and anon-limited data integrity. Latest no-deploy email/messaging authorization batch passed focused tests, message smoke, typecheck, quiet lint, file-size guard, diff check, and production build. Remaining validation gaps are secure service-role/RLS live proof, authenticated collaborator-role live mutation proof, and external OpenAI key rotation.
...
14. `PARTIAL` - Performance and query safety.
    ...
    Current evidence: explicit projection work reduced overfetching, and `npm run proof:v1:performance-budget` now runs in `test:launch` plus CI hardpass after production build so oversized route chunks cannot drift silently. Guest, message, guest-photo, RSVP board, coordinator, overview, registry, seating, and itinerary dashboard service reads now have explicit high-water caps for high-cardinality guests/messages/events/invitations/albums/media support rows, and `dashboardDataBoundary.test.ts` guards those constants plus `.limit(...)` calls. Pagination/query-efficiency audit remains for true beyond-cap product pagination.
...
15. `PARTIAL` - Asset footprint.
   ...
   Current evidence: `npm run guard:assets` now budgets production-copied `public/` assets at the current footprint, is wired into `test:launch`, `proof:v1:test-lanes`, and CI hardpass, and fails on growth. The total public asset cap is tightened to 210000 KiB and the per-file cap is tightened to 5000 KiB against the current 209433 KiB / 4788 KiB footprint. The guard also reports and budgets heavy top-level directories: `template-previews-gif` 141000 KiB, `preview-photos` 42000 KiB, `variant-previews` 12000 KiB, `photos` 11000 KiB, and `template-previews` 4500 KiB, while rejecting new unbudgeted public directories over 500 KiB. Existing template GIFs still need a CDN/object-storage or optimized-thumbnail strategy.
...
- 2026-05-06 1:25 PM PT - No-deploy live email/messaging unauthenticated denial proof:
  - Resolved in this batch: added `scripts/v1-proof-email-messaging-authorization.mjs` and wired it as `npm run proof:v1:email-messaging-authorization`.
  - Live authorization proof: with no `Authorization` bearer, deployed `process-email-queue`, `queue-guest-followups`, and `send-bulk-message` returned `401`; deployed `send-wedding-email` returned `401` at the platform/function authorization boundary for direct RSVP email relay input. No send or queue mutation path was authorized.
  - Proof-lane hardening: `src/lib/launchEdgeFunctions.test.ts` now guards that the proof script and package command keep covering `process-email-queue`, `queue-guest-followups`, `send-bulk-message`, and `send-wedding-email`. `scripts/v1-proof-board.mjs` now lists the live denial proof in the comms-center automated proof set, and `src/lib/proofBoardFreshness.test.ts` guards that board contract.
  - Validation passed: first sandbox proof attempt failed only on blocked network fetches; rerun with approved network access passed. `npm test -- --run src/lib/launchEdgeFunctions.test.ts src/lib/plannerAccess.test.ts` (39/39), `npm test -- --run src/lib/proofBoardFreshness.test.ts src/lib/launchEdgeFunctions.test.ts src/lib/plannerAccess.test.ts` (40/40), `npm run smoke:messages`, `npm run proof:v1:board:md`, `npm run typecheck -- --pretty false`, `npm run lint -- --quiet`, `npm run guard:file-size`, `git diff --check`, and `npm run build` all passed.
  - Launch status: unchanged. This narrows live email/messaging authorization risk without deploying. Authenticated owner/planner/coordinator/viewer live mutation proof, secure service-role/RLS proof, and external OpenAI key rotation remain required.
- 2026-05-06 1:32 PM PT - No-deploy live photo/media service-role unauthenticated denial proof:
  - Resolved in this batch: added `scripts/v1-proof-service-role-authorization.mjs` and wired it as `npm run proof:v1:service-role-authorization`.
  - Live authorization proof: with no `Authorization` bearer, deployed `photo-album-create`, `photo-album-manage`, `photo-upload-moderate`, `photo-export-manifest`, and `photo-analyze-batch` returned `401` before service-role album creation, mutation, media moderation, export/storage signing, or AI analysis writes could run.
  - Proof-lane hardening: `src/lib/launchEdgeFunctions.test.ts` now guards the proof command and function coverage. `scripts/v1-proof-board.mjs` now lists the live service-role/media denial proof in AI/photo proof commands and automated proof, and `src/lib/proofBoardFreshness.test.ts` guards that board contract.
  - Validation passed: first sandbox proof attempt failed only on blocked network fetches; rerun with approved network access passed. `npm test -- --run src/lib/launchEdgeFunctions.test.ts src/lib/proofBoardFreshness.test.ts src/lib/plannerAccess.test.ts` (40/40) and `npm run proof:v1:board:md` passed.
  - Launch status: unchanged. This narrows deployed photo/media service-role authorization risk without deploying. Authenticated owner/planner/coordinator/viewer live mutation proof and secure service-role/RLS cross-table/storage proof remain required.
- 2026-05-06 1:37 PM PT - No-deploy public subresource access-gate coverage proof:
  - Resolved in this batch: added `scripts/v1-proof-public-access-coverage.mjs` and wired it as `npm run proof:v1:public-access-coverage`.
  - Concrete bug fixed: the first proof run caught `photo-upload` because the slug path used `canReadPublicSubresource` and then later re-applied a stale `!tokenHash && !site.is_published` shortcut before Drive backup resolution. That could block password/invite-mode slug uploads after the shared gate had already authorized access. The stale shortcut was removed and the second site read now selects only the Drive backup fields it needs.
  - Proof-lane hardening: `src/lib/launchEdgeFunctions.test.ts` now guards the proof command and the `photo-upload` shared-gate/no-published-shortcut contract. `scripts/v1-proof-board.mjs` now lists the public-access coverage proof in the public-site trust automated proof set, and `src/lib/proofBoardFreshness.test.ts` guards that board contract.
  - Validation passed: `npm run proof:v1:public-access-coverage` passed after the fix across `guest-contact-lookup`, `guest-hub-config`, `guest-hub-track`, `guest-prospect-submit`, `guest-recap-config`, `guestbook-submit`, `photo-upload`, `public-itinerary-by-slug`, `public-registry-items`, `public-site-rsvp-submit`, `vault-entry-submit`, and `vault-upload-google-drive`. `npm test -- --run src/lib/launchEdgeFunctions.test.ts src/lib/proofBoardFreshness.test.ts src/lib/publicSiteAccess.test.ts` (32/32) and `npm run proof:v1:board:md` passed.
  - Launch status: unchanged. This is local source hardening and no deploy was run; production will need an approved deploy before the `photo-upload` slug-mode fix is live.
- 2026-05-06 1:41 PM PT - No-deploy public access proof hardpass wiring:
  - Resolved in this batch: wired `npm run proof:v1:public-access-coverage` into `npm run test:launch` immediately after `test:security` and before build/guard proof.
  - CI/proof-lane hardening: `.github/workflows/ci-hardpass.yml` now runs the public access coverage proof as a named hardpass step, and `scripts/v1-proof-test-lanes.mjs` now fails if the command, launch-lane placement, or CI step is removed.
  - No feature loss: this is release-lane hardening only; runtime behavior is unchanged beyond the already-recorded `photo-upload` source fix.
  - Validation passed: `npm run proof:v1:public-access-coverage`, `npm run proof:v1:test-lanes` (10/10 script contracts), and the full updated `npm run test:launch` chain passed. The launch chain ran typecheck, quiet lint, `test:security` (226/226), public-access coverage proof, file-size guard, asset guard, production build, performance-budget proof, and proof-board markdown generation.
  - Launch status: unchanged. This is local-only hardening and no deploy was run.
- 2026-05-06 1:45 PM PT - No-deploy public asset directory budget guard:
  - Resolved in this batch: `scripts/check-asset-budget.mjs` now reports top-level public asset directory totals and enforces explicit directory budgets for `template-previews-gif`, `preview-photos`, `variant-previews`, `photos`, and `template-previews`.
  - Asset hardening: new unbudgeted public directories over 500 KiB now fail the guard, so large media buckets cannot appear silently even when individual files stay below the per-file cap.
  - Current budget proof: `npm run guard:assets` passes at 209433 KiB total. Largest directory buckets are `public/template-previews-gif` 140502 KiB under a 141000 KiB cap, `public/preview-photos` 41847 KiB under a 42000 KiB cap, `public/variant-previews` 11911 KiB under a 12000 KiB cap, `public/photos` 10551 KiB under an 11000 KiB cap, and `public/template-previews` 4327 KiB under a 4500 KiB cap.
  - No feature loss: this is guard/reporting hardening only; existing preview GIFs, preview photos, variant previews, and public assets remain available.
  - Validation passed: `npm run guard:assets` and the full updated `npm run test:launch` chain passed. The launch chain ran typecheck, quiet lint, `test:security` (226/226), public-access coverage proof, file-size guard, tightened asset directory guard, production build, performance-budget proof, and proof-board markdown generation.
- 2026-05-06 1:48 PM PT - No-deploy dashboard high-cardinality query caps:
  - Resolved in this batch: added explicit service-layer high-water caps to guest dashboard guest/event/invitation reads, message dashboard history/recipient/event/invitation reads, and guest-photo event/album reads.
  - Query safety hardening: `src/lib/dashboardDataBoundary.test.ts` now guards the cap constants and required `.limit(...)` calls for those high-cardinality service reads alongside the existing explicit-projection/no-direct-page-table-access contracts.
  - No feature loss: existing dashboard pages still call the same service functions and keep current sorting/projections; this bounds worst-case loads while leaving the true beyond-cap pagination work tracked separately.
  - Validation passed: `npm test -- --run src/lib/dashboardDataBoundary.test.ts src/pages/dashboard/guestPhotoSharingService.test.ts src/pages/dashboard/settings/settingsSiteData.test.ts src/pages/dashboard/messages/messageDashboardUtils.test.ts` (34/34), `npm run typecheck -- --pretty false`, `npm run lint -- --quiet`, `npm run guard:file-size`, `npm run guard:assets`, `git diff --check`, and `npm run build`.
  - Launch status: unchanged. This is local-only hardening and no deploy was run.
- 2026-05-06 1:52 PM PT - No-deploy dashboard high-cardinality query caps continuation:
  - Resolved in this batch: added explicit high-water caps to RSVP board guest/event/invitation reads, coordinator guest/event/invitation reads, overview guest reads, registry item reads, seating itinerary/lookup assignment reads, and itinerary guest-picker invitation reads.
  - Query safety hardening: `src/lib/dashboardDataBoundary.test.ts` now guards the second wave of cap constants and `.limit(...)` calls across those service modules.
  - No feature loss: service APIs, projections, sorting, and dashboard callers are unchanged; this only bounds worst-case service reads while true beyond-cap pagination remains tracked separately.
  - Validation passed: `npm test -- --run src/lib/dashboardDataBoundary.test.ts src/pages/dashboard/seating/seatingDashboardUtils.test.ts src/pages/dashboard/messages/messageDashboardUtils.test.ts src/pages/dashboard/registry/registryTypes.test.ts` (49/49), `npm run typecheck -- --pretty false`, `npm run lint -- --quiet`, `npm run guard:file-size`, `npm run guard:assets`, `git diff --check`, and `npm run build`.
  - Launch status: unchanged. This is local-only hardening and no deploy was run.
- 2026-05-06 1:58 PM PT - No-deploy onboarding AI service-role usage trust fix:
  - Resolved in this batch: `supabase/functions/onboarding-ai-orchestrate/index.ts` no longer trusts a browser-supplied `siteId` for service-role usage attribution or rate-limit site context.
  - Service-role hardening: the function now resolves `verifiedUsageSiteId` by reading the bearer token with `admin.auth.getUser(token)`, confirming owner access through `wedding_sites.user_id`, and falling back to collaborator membership through `wedding_site_collaborators` before writing `internal_ai_usage_events.wedding_site_id`.
  - No feature loss: anonymous onboarding still returns deterministic or model-backed onboarding output and remains IP/subject rate-limited; unverified site ids simply stop being attached to internal usage rows.
  - Validation passed: `npm test -- --run src/lib/launchEdgeFunctions.test.ts src/lib/aiProviderKeySecurity.test.ts` (36/36), `npm run typecheck -- --pretty false`, `npm run lint -- --quiet`, `npm run guard:file-size`, `git diff --check`, `npm run guard:assets`, and `npm run build`.
  - Launch status: unchanged. This is local source hardening and no deploy was run; production needs an approved deploy before this Edge Function fix is live.
- 2026-05-06 2:01 PM PT - No-deploy service-role disposition category hardening:
  - Resolved in this batch: `docs/service-role-authorization-disposition-2026-05-05.md` now separates public/optional-auth rate-limited helpers from owner/collaborator service-role functions instead of overclaiming auth requirements for `log-client-error`, `onboarding-ai-orchestrate`, and `vendor-profile-preview`.
  - Proof hardening: `src/lib/serviceRoleAuthorizationDisposition.test.ts` now parses the disposition categories and requires every current service-role Edge Function to appear in exactly one category, with explicit expected lists for owner/collaborator, public submission, public/optional-auth helper, and internal/provider scopes.
  - No feature loss: documentation and proof contracts changed only; runtime Edge Function behavior is unchanged.
  - Validation passed: `npm test -- --run src/lib/serviceRoleAuthorizationDisposition.test.ts src/lib/launchEdgeFunctions.test.ts` (30/30), `npm run typecheck -- --pretty false`, `npm run lint -- --quiet`, `npm run guard:file-size`, `npm run guard:assets`, `git diff --check`, and `npm run build`.
  - Launch status: unchanged. This is local proof/disposition hardening and no deploy was run; secure service-role/RLS live proof remains open.
- 2026-05-06 2:02 PM PT - No-deploy full local launch-lane checkpoint:
  - Validation passed: `npm run test:launch` completed green after the onboarding service-role trust fix and disposition category hardening.
  - Lane coverage: typecheck, quiet lint, `test:security` (227/227), public subresource access-gate coverage proof, file-size guard, tightened asset guard, production build, performance-budget proof, and proof-board markdown generation.
  - Performance proof: 0 failures and 3 review chunks remain under the hard cap: registry 325.27 KiB, nameChangeService 288.54 KiB, and Planning 253.86 KiB.
  - Proof board regenerated at 2026-05-06 2:02 PM PT and still lists the active ungated launch blockers as secure service-role/RLS live proof and email/messaging live authorization proof.
  - Launch status: unchanged. This was validation only and no deploy was run.
- 2026-05-06 2:05 PM PT - No-deploy Google Drive OAuth state signing hardening:
  - Resolved in this batch: `google-drive-auth-start` now signs Google Drive OAuth state with the shared HMAC session-token helper, and `google-drive-auth-callback` rejects unsigned or tampered state before exchanging/writing Drive tokens with the service-role client.
  - Service-role hardening: callback state now carries `scope: "google_drive_oauth"`, `siteId`, `userId`, and timestamp inside a verified token using `GOOGLE_DRIVE_STATE_SECRET` or the service-role key fallback; the callback no longer parses trusted identity from `JSON.parse(atob(stateRaw))`.
...
- 2026-05-06 2:22 PM PT - No-deploy vendor fallback draft image hardening:
  ...
  - Shared link hardening: `getSafePublicInstagramUrl(...)` now rejects credentialed Instagram URLs, and shared public web/image helpers now reject credentialed, localhost, metadata, private IPv4, internal, `.local`, and `.test` hosts.
  - Proof hardening: added `src/lib/vendorProfiles.test.ts` to prove fallback drafts keep hero/gallery images empty, avoid `image.thum.io`, preserve safe public website/Instagram URLs, drop unsafe website/social inputs, and sanitize unsafe direct create payloads before insert. `src/sections/publicLinks.test.ts` now guards credentialed/private-host web, image, and Instagram URL rejection.
...
- 2026-05-06 2:53 PM PT - No-deploy preview reserved-host SSRF hardening:
  - Resolved in this batch: registry preview, registry preview image-resource normalization, and vendor profile preview now reject reserved `.invalid` and `.example` hostnames in addition to local/internal/test/private/metadata hosts.
  - SSRF hardening: preview source fetches and nested `images.weserv.nl` image proxy targets now treat these reserved hostnames as non-public, aligning Edge preview guards with the shared public-link sanitizer.
  - No feature loss: safe public product/vendor URLs, public image URLs, and existing public image proxy URLs remain allowed.
  - Validation passed: `npm test -- --run src/lib/registryPreviewUrlNormalizer.test.ts src/lib/launchEdgeFunctions.test.ts` (66/66), `npm run typecheck -- --pretty false`, `npm run lint -- --quiet`, `npm run guard:file-size`, `npm run guard:assets`, `git diff --check`, `npm run build`, and `npm run proof:v1:ai-product-readiness` (23/23).
