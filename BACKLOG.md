...
| Field | Current State |
| --- | --- |
| Current date/time | `2026-05-11 07:21 PM PDT` |
| Branch | `codex/v1-finish-hard-gates-3` |
| Latest Git SHA | `eb515395` |
| Latest commit message | `Move registry and message dashboard writes behind RPCs` |
...
| Current proof state | `npm test` passed `537/537` files and `3321/3321` tests. The local gate (`typecheck`, `lint`, `build`, `test:security`, `public-access-coverage`, `board:md`, `git diff --check`) is green. `test:smoke` passed with live Supabase access. The secure proof bundle (`service-role-authorization`, `email-messaging-authorization`, `launch-closeout`) remains green. Live production proofs after the blocker-fix deploy are green for `canonical-smoke`, `public-quality`, `guests-rsvp-ops`, `guest-lookup-scope`, `collaborator-runtime`, and `client-rls-matrix`, with direct guest, planning, and seating writes now proven role-scoped while direct timeline/settings writes remain denied without permission. New local-only proof is green for guest-core, planning, seating, payment-gate timing, invitation/import/RSVP RPC hardening, itinerary invitation RPC reuse, registry owner-write RPC hardening, message/coordinator write RPC hardening, and settings/overview owner-write RPC hardening (`guestService.test`, `planningService.test`, `seatingService.test`, `itineraryService.test`, `itineraryQueryBounds.test`, `registryService.test`, `messageService.boundary.test.ts`, `coordinatorService.test.ts`, `settingsSiteData.test.ts`, `overviewService.test.ts`, `ProtectedRoute.test.tsx`, `typecheck`, `lint`, `build`). The remaining proof gaps in this cluster are the undeployed local RPC batches gated behind migration apply/deploy plus `LIVE_GUEST_DASHBOARD_SETTINGS_RPCS=1`. GitHub Actions `Release Launch Gate` is green on run `25705683563`, with the focused launch-critical proof bundle and mandatory strict RSVP smoke. |
| Current deployment state | Frontend is live at [dayof.love](https://dayof.love) on exact runtime Git SHA `f0cbf841` via verified Vercel production deploy `dpl_386dKTNkTVK95UfwJj9qEtnH1b8q`. The latest pushed branch head `eb515395` adds local post-launch hardening only and does not change the live frontend runtime. `submit-rsvp` is live with the serialized capacity path and the migration is applied remotely. New local-only migrations `20260511200000_guest_dashboard_settings_rpcs.sql`, `20260511211500_planning_seating_write_rpcs.sql`, `20260511220000_guest_core_write_rpcs.sql`, `20260511233000_guest_invitation_rsvp_rpcs.sql`, `20260511234500_registry_write_rpcs.sql`, `20260512001000_message_coordinator_write_rpcs.sql`, and `20260512012000_settings_overview_write_rpcs.sql` are not applied remotely yet. Public vault contribution remains outside the launch baseline because the live route still fails closed and the function inventory does not confirm `vault-contribution-public`. |
| Current next actions | apply/deploy the seven local RPC batches, rerun `collaborator-runtime` / `client-rls-matrix` with `LIVE_GUEST_DASHBOARD_SETTINGS_RPCS=1`, then continue on remaining non-guest direct-write surfaces |
...
1. Apply and deploy the seven local RPC batches (`20260511200000_guest_dashboard_settings_rpcs.sql`, `20260511211500_planning_seating_write_rpcs.sql`, `20260511220000_guest_core_write_rpcs.sql`, `20260511233000_guest_invitation_rsvp_rpcs.sql`, `20260511234500_registry_write_rpcs.sql`, `20260512001000_message_coordinator_write_rpcs.sql`, and `20260512012000_settings_overview_write_rpcs.sql`), then rerun `proof:v1:collaborator-runtime` and `proof:v1:client-rls-matrix` with `LIVE_GUEST_DASHBOARD_SETTINGS_RPCS=1`.
