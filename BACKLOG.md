...
| Current proof state | Launch-critical runtime proof remains green: `npm test`, `typecheck`, `lint`, `build`, `test:security`, `public-access-coverage`, `service-role-authorization`, `email-messaging-authorization`, `launch-closeout`, `canonical-smoke`, `public-quality`, `guests-rsvp-ops`, `guest-lookup-scope`, `collaborator-runtime`, and `client-rls-matrix` are green on the current live baseline. The post-launch hardening lane is broader again: `npm run proof:v1:client-write-inventory` is still `PASS`, and the live reruns with `LIVE_GUEST_DASHBOARD_SETTINGS_RPCS=1` now also prove planner `itinerary_event_write` allow + registry deny, settings `section_write` allow + registry deny, and photos `wedding_site_vault_provider_patch` allow + dashboard message deny, in addition to the earlier guest, planning task/message, settings patch, registry item, photos/vault-config, and coordinator seating/Q&A/check-in/media lanes. This wave found and fixed three real production RPC/runtime defects before the proof went green: missing `itinerary_events.dress_code`, missing `itinerary_events.notes`, and create-path bugs in `itinerary_event_write` (`NULL id` plus text/time coercion drift) and `section_write` (explicit-id create path misread as missing-row update). Remote repair landed through migrations `20260512040000_reconcile_itinerary_dress_code_column.sql`, `20260512040500_reconcile_itinerary_runtime_columns.sql`, `20260512041000_fix_itinerary_event_write_time_types.sql`, `20260512041500_fix_itinerary_event_write_ids.sql`, and `20260512042000_fix_section_write_create_with_explicit_id.sql`. Focused proof is green for `itineraryRpcSafety.test.ts`, `sectionRpcSafety.test.ts`, `collaboratorPermissionRlsProof.test.ts`, `clientRlsMatrixProofScript.test.ts`, `typecheck`, and `git diff --check`. GitHub Actions `Release Launch Gate` remains green on run `25705683563`. |
| Current deployment state | Frontend is live at [dayof.love](https://dayof.love) on exact runtime Git SHA `f0cbf841` via verified Vercel production deploy `dpl_386dKTNkTVK95UfwJj9qEtnH1b8q`. `submit-rsvp` is live with the serialized capacity path and the database now includes the full RPC migration sweep through `20260512031500_seating_assignment_version_rpcs.sql` plus forward repair migrations `20260511212626_fix_wedding_site_settings_patch_types.sql`, `20260512040000_reconcile_itinerary_dress_code_column.sql`, `20260512040500_reconcile_itinerary_runtime_columns.sql`, `20260512041000_fix_itinerary_event_write_time_types.sql`, `20260512041500_fix_itinerary_event_write_ids.sql`, and `20260512042000_fix_section_write_create_with_explicit_id.sql`. The live collaborator/client-RLS matrix now includes guest, planning, itinerary, settings, sections, seating, messages, registry, photos/vault, and coordinator permission lanes. Public vault contribution remains outside the launch baseline because the live route still fails closed and the function inventory does not confirm `vault-contribution-public`. |
| Current next actions | broaden the live client-RLS matrix across any remaining owner-only or non-guest RPC surfaces beyond the current guest/planning/itinerary/settings/registry/photos/coordinator baseline, keep the no-direct-client-write inventory current after future runtime write-surface changes, and resolve deferred lanes only when they are intentionally brought into the launch baseline |
...
- `P2 Client-side Supabase write surface is still too broad`
  ...
  - note:
    ...
    - the remaining gap is broader matrix breadth beyond the current guest/planning/seating baseline
  - direction:
...
- `P2 Live RLS proof should be expanded further`
  ...
  - note:
    ...
    - it now also proves direct guest, planning, itinerary, settings, sections, registry, seating, coordinator, message, photo, vault-config, and vault-provider RPC lanes remain permission-scoped while direct timeline/settings and other ungranted writes stay denied without the matching permission set
    - guest-dashboard settings plus guest-core and planning/seating core writes are now routed through local-only RPC batches
    - broader owner-only and remaining non-guest RPC lane coverage is still worth adding because the client-heavy mutation surface is now centralized rather than eliminated
...
| `npm run proof:v1:client-rls-matrix` | `LIVE PASS` | `production + browser runtime` | `2026-05-11` | Aggregates live anon guest-contact scope, public RSVP scope, owner/collaborator viewer-deny plus planner/coordinator/registry/settings/photos-allow runtime proof, direct guest/planning/seating write allow/deny coverage, planner message + itinerary RPC allow with registry RPC deny, settings patch/section RPC allow + registry RPC deny, registry RPC allow + message RPC deny, photos vault-config/vault-provider RPC allow + dashboard message RPC deny, coordinator Q&A/check-in/media RPC allow + dashboard message RPC deny, and the guest-dashboard settings RPC lane with `LIVE_GUEST_DASHBOARD_SETTINGS_RPCS=1` |
...
1. Expand live client-RLS coverage into any remaining owner-only or non-guest RPC surfaces beyond the current guest/planning/itinerary/settings/registry/photos/coordinator baseline.
