...
| Field | Current State |
| --- | --- |
| Current date/time | `2026-05-11 09:37 PM PDT` |
| Branch | `codex/v1-finish-hard-gates-3` |
| Latest Git SHA | `f9f18fc4` |
| Latest commit message | `Fix settings RPC type mismatch and expand live settings RLS proof` |
...
| Current proof state | Launch-critical runtime proof remains green: `npm test`, `typecheck`, `lint`, `build`, `test:security`, `public-access-coverage`, `service-role-authorization`, `email-messaging-authorization`, `launch-closeout`, `canonical-smoke`, `public-quality`, `guests-rsvp-ops`, `guest-lookup-scope`, `collaborator-runtime`, and `client-rls-matrix` are green on the current live baseline. The post-launch hardening lane is broader now: `supabase db push` applied the RPC sweep through `20260512031500_seating_assignment_version_rpcs.sql`, `npm run proof:v1:client-write-inventory` is `PASS`, and the live reruns with `LIVE_GUEST_DASHBOARD_SETTINGS_RPCS=1` kept `collaborator-runtime` and `client-rls-matrix` green while explicitly proving planner message RPC allow + registry RPC deny, settings RPC allow + registry RPC deny, registry RPC allow + message RPC deny, and coordinator seating/Q&A/check-in/media RPC allow + dashboard message RPC deny. This wave also found and fixed a real `wedding_site_settings_patch` PostgreSQL `CASE` type mismatch on `active_template_id`, repaired remotely by migration `20260511212626_fix_wedding_site_settings_patch_types.sql`. Focused proof is green for the refreshed proof scripts and settings regression guard (`settingsErrorSafety.test.ts`, `collaboratorPermissionRlsProof.test.ts`, `clientRlsMatrixProofScript.test.ts`, `typecheck`, `git diff --check`). GitHub Actions `Release Launch Gate` remains green on run `25705683563`. |
| Current deployment state | Frontend is live at [dayof.love](https://dayof.love) on exact runtime Git SHA `f0cbf841` via verified Vercel production deploy `dpl_386dKTNkTVK95UfwJj9qEtnH1b8q`. `submit-rsvp` is live with the serialized capacity path and the database now includes the full RPC migration sweep through `20260512031500_seating_assignment_version_rpcs.sql` plus forward repair migration `20260511212626_fix_wedding_site_settings_patch_types.sql`. The live collaborator/client-RLS matrix now includes guest, planning, settings, seating, messages, registry, and photos permission lanes. Public vault contribution remains outside the launch baseline because the live route still fails closed and the function inventory does not confirm `vault-contribution-public`. |
| Current next actions | broaden the live client-RLS matrix across any remaining owner-only or non-guest RPC surfaces, keep the no-direct-client-write inventory current after future runtime write-surface changes, and resolve deferred lanes only when they are intentionally brought into the launch baseline |
...
- `P2 Live RLS proof should be expanded further`
  ...
  - note:
    ...
    - it now also proves direct guest, planning, settings, registry, seating, coordinator, message, and photo RPC lanes remain permission-scoped while direct timeline/settings and other ungranted writes stay denied without the matching permission set
...
| `npm run proof:v1:client-rls-matrix` | `LIVE PASS` | `production + browser runtime` | `2026-05-11` | Aggregates live anon guest-contact scope, public RSVP scope, owner/collaborator viewer-deny plus planner/coordinator/registry/settings-allow runtime proof, direct guest/planning/seating write allow/deny coverage, planner message RPC allow + registry RPC deny, settings RPC allow + registry RPC deny, registry RPC allow + message RPC deny, coordinator Q&A/check-in/media RPC allow + dashboard message RPC deny, and the guest-dashboard settings RPC lane with `LIVE_GUEST_DASHBOARD_SETTINGS_RPCS=1` |
...
| `npm run proof:v1:collaborator-runtime` | `LIVE PASS` | `production` | `2026-05-11` | Owner invite/accept flow, viewer deny + planner/coordinator allow runtime proof, direct guest/planning/seating write allow/deny coverage, and the guest-dashboard settings RPC lane are green |
...
1. Expand live client-RLS coverage into any remaining non-guest direct-write surfaces that stay client-driven after the RPC apply sweep.
