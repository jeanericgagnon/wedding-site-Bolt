...
13. `PARTIAL` - Remove direct Supabase calls from pages.
    ...
    Current evidence: many broad `select("*")` projections were replaced by explicit projections; the last page-level Supabase mutation/RPC calls in `AcceptCollaboratorInvite.tsx` and `Guests.tsx` now run through helper services instead of direct TSX calls; `SiteView.tsx` now routes public itinerary and registry Edge Function reads through `src/pages/siteViewService.ts` instead of owning direct `supabase.functions.invoke(...)` calls inline; `VaultContribute.tsx` now routes public vault config reads plus vault upload/submission Edge Function calls through `src/pages/vaultContributionService.ts` instead of owning direct `supabase.functions.invoke(...)` calls inline; `Overview.tsx` now routes site lookup, guest/count hydration, interactive suggestion/vote reads, builder field edits, and draft-refresh seed reads through `src/pages/dashboard/overviewService.ts` instead of owning those `wedding_sites`, `guests`, `interactive_suggestions`, and `interactive_votes` queries inline; `Itinerary.tsx` now routes wedding-data/sections mirror sync plus repeated active-site auth lookup through `src/pages/dashboard/itineraryService.ts` instead of owning those cross-table `wedding_sites` and `sections` writes and repeated `supabase.auth.getUser()` calls inline; `Messages.tsx` now routes bulk-send auth token lookup through `src/pages/dashboard/messages/messageService.ts` instead of owning direct session reads inline; `Seating.tsx` now routes auth-session refresh retry through `src/pages/dashboard/seating/seatingService.ts` instead of owning direct session refresh calls inline; `Vault.tsx` now routes entry-link resolution plus Google Drive health/start/callback function invokes through `src/pages/dashboard/vaultService.ts` instead of owning direct `supabase.functions.invoke(...)` calls inline; `GuestPhotoSharing.tsx` now routes dashboard user resolution, session refresh retry, and owner actor-id lookup through `src/pages/dashboard/guestPhotoSharingService.ts` instead of owning direct `supabase.auth.getSession()`, `supabase.auth.getUser()`, and `supabase.auth.refreshSession()` calls inline; `Login.tsx` now routes OAuth prime-session lookup through `src/pages/loginService.ts` instead of owning direct session reads inline while still keeping the page-level auth-state listener; `AcceptCollaboratorInvite.tsx` now routes invite-claim session presence checks through `src/pages/acceptCollaboratorInviteService.ts` instead of owning direct session reads inline; `Guests.tsx` now routes guest check-in auth retry refresh through `src/pages/dashboard/guests/guestService.ts` instead of owning direct session refresh calls inline; `Signup.tsx` now routes Google OAuth start and email sign-up/sign-in fallback through `src/pages/signupService.ts` instead of owning those auth calls inline; `Login.tsx` now routes password sign-in, Google OAuth start, and reset-email submission through `src/pages/loginService.ts` instead of owning those auth calls inline; `AcceptCollaboratorInvite.tsx` now routes invited account sign-in and sign-up fallback through `src/pages/acceptCollaboratorInviteService.ts` instead of owning those auth calls inline; `QuickStart.tsx`, `GuidedSetup.tsx`, and `WeddingStatus.tsx` now route their authenticated onboarding user lookup through `src/pages/onboarding/onboardingService.ts` instead of directly calling `supabase.auth.getUser()` inline; `Settings.tsx` now routes authenticated email lookup, current-password verification, and password update through `src/pages/dashboard/settings/settingsSiteData.ts` instead of owning those auth calls inline; full service-layer migration still remains.
