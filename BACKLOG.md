...
- 2026-05-06 2:53 PM PT - No-deploy preview reserved-host SSRF hardening:
  - Resolved in this batch: registry preview, registry preview image-resource normalization, vendor profile preview, and local vendor profile fallback sanitization now reject reserved `.invalid` and `.example` hostnames in addition to local/internal/test/private/metadata hosts.
  - SSRF hardening: preview source fetches, nested `images.weserv.nl` image proxy targets, and browser-side fallback vendor drafts now treat these reserved hostnames as non-public, aligning preview guards with the shared public-link sanitizer.
